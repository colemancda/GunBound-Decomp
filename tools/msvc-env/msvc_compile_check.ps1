# Real per-file compile (no /Zs shortcut - actually generates .obj) of every
# raw-ported .c file with MSVC 7.10. Slower than the syntax-only pass but
# catches anything /Zs's parse-only mode would miss (codegen-stage errors).
...
New-Item -ItemType Directory -Force -Path build\objcheck | Out-Null
...
foreach ($f in $files) {
    ...
    $objOut = "build\objcheck\$($f.BaseName).obj"
    $out = & $cl /c /nologo /I include\msvc /I include /O1 /Fo"$objOut" $rel 2>&1
...
$results | Export-Csv -NoTypeInformation -Path tools\msvc_compile_check_results.csv
