...
def obj_path_for(build_dir, src_rel):
    ...
    obj_name = obj_name[:-2] + ".obj" if obj_name.endswith(".c") else obj_name + ".obj"
...
def run_diff(build_dir, src_rel, va_hex, hint):
...
<head>
<meta charset="utf-8">
<title>WinMain to Server List: disassembly diff</title>
<style>
{font_face}
:root {{
...
<body>
<header>
  <h1>WinMain &rarr; Server List: disassembly diff</h1>
  <p class="sub">Instruction-level comparison of every function on the startup path (original <code>GunBound.gme</code> vs. the current MSVC 7.10 recompile), scored with <code>tools/asm-differ</code>. Low scores are largely expected: unresolved relocations in an unlinked <code>.obj</code> inflate the diff, and MSVC's own register allocation rarely reproduces byte-for-byte, even for logically correct code &mdash; click a row to inspect.</p>
  <div class="stats">
...
<footer>
  Generated by <code>tools/gen_diff_report.py</code> (asm-differ <code>-e</code> ELF-symbol mode) against objects from <code>build/link_attempt/</code>.
</footer>
<script>
