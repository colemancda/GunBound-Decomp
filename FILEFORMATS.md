...
- **Per-record layout — now confirmed directly from `LoadGameDataFiles`'s
  ...
  | `0x30` | 2 | `ushort` field — **decoded: the packed shelf-icon code** | Read as a genuine 16-bit value and checksummed — **correction from the previous pass**, which had read this as two separate bytes (a duplicate type-ID byte at `0x30` plus a `0xff` "marker" byte at `0x31`); the real code treats it as one `ushort`. **Decoded this pass** by decompressing the real `orig/itemdata.dat` (100×308-byte records, 13 populated) and cross-referencing every populated record's `0x30` value against the Ready Room's `g_awItemIconTable` shelf-icon table (see ARCHITECTURE.md's Ready Room section): every real record's `0x30` value appears verbatim in that 40-entry table, using the identical low-byte=icon-pair/high-byte=sheet-select packing. So `0x30` is this item's own pre-baked shelf-icon code, not a separate value — though it is *not* what indexes `g_awItemIconTable` at runtime (ruled out: type ID and record-slot-number both fail as the index; see ARCHITECTURE.md for the cross-reference). |
