 ...
 * edit control (g_sharedTextInputControl: the CreateWindowExA edit box built by
 ...
 * through to `*(char *)(g_sharedTextInputControl + 8)` - and g_sharedTextInputControl is NULL on the
 ...
 * slot, the whole body short-circuits, and g_sharedTextInputControl is never touched.
 *
 * KNOWN DIVERGENCE (pre-existing, out of scope for bring-up, same class as
 * the DAT_00e9c104 note in globals.c): g_sharedTextInputControl is read but never written
...

{
  ...
  if ((*(int *)(param_1 + 8) != 0) &&
     ((lpString = (LPSTR)(*(int *)(param_1 + 8) + 0x48), *(char *)(g_sharedTextInputControl + 8) == '\0' ||
      (iVar1 = GetWindowTextA(*(HWND *)(g_sharedTextInputControl + 4),lpString,0x80), iVar1 == 0)))) {
    *lpString = '\0';
  }
  return;
}
