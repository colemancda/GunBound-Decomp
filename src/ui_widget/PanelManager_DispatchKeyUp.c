/* PanelManager_DispatchKeyUp - 0x0050f260 in the original binary.
 *
 * The panel manager's WM_KEYUP dispatch - the exact sibling of
 * PanelManager_DispatchKeyDown (WM_KEYDOWN) but calling each panel's key-up virtual
 * (vtable slot 6, +0x18). Same dropped-`this` + dropped-return bug and
 * fix - see PanelManager_DispatchKeyDown.c's header. Confirmed at orig 0x50f272-0x50f27d
 ...
 * dummy-EDX idiom - see PanelManager_DispatchKeyDown.c. */
typedef undefined1 (__fastcall *PanelKeyHandlerFn)(void *thisPtr, int dummyEDX, undefined4 key);

undefined1 __thiscall PanelManager_DispatchKeyUp(int param_1,undefined4 param_2)
