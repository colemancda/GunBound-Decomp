/* HandleBackgroundActiveObjectMouseDown - 0x0040ce50 in the original binary.
 *
 ...
 * FindActiveObjectAt.c's header for the full trace of all 5). Unlike its 3
 * siblings, `param_1` here doubles as BOTH the registry pointer passed
 * into FindActiveObjectAt's EAX slot AND this function's own struct pointer
...

{
  ...
  iVar1 = FindActiveObjectAt((void *)param_1,mouseX,mouseY);
  if (iVar1 == 0) {
    return 0;
  }
