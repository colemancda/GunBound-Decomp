/* FUN_0048f300 - 0x0048f300 in the original binary.
 *
 * No confirmed real name/purpose. Raw/near-verbatim port of Ghidra's
 * decompiler output, not hand-verified. See src/README.md's "Raw/
 * verbatim ports" section for status.
 *
 * DROPPED-CELL FIX (2026-08-11, guard sweep): recovered the dropped
 * guard-cell pointer at all 50 argless PeekPacketChecksumState() sites
 * and all 6 one-arg EncodeOutgoingPacketField(value) sites, per
 * tools/guard_worklist.json (both families "clean", 50/50 + 6/6), with
 * every cell re-derived from the original disasm (capstone dump
 * 0x48f300-0x490771) - the decompile is goto-shuffled (LAB_0048fe95/
 * LAB_00490184 tail blocks) and 4 of the json's encode cells were null.
 * Landmark anchors: EncodeChecksumPairDiff/0x40aca0 arg triples,
 * FUN_00436860/0x436860, InitGuardedChecksumSlot/0x40ada0, the
 * g_valueGuardKeyTable rand-scramble loops, and the local_2480[0]/local_2498
 * frame slots ([esp+0x30]/[esp+0x18] at frame base). Notable
 * recoveries: NINE chained-return cells - EncodeChecksumPairDiff,
 * EncodeChecksumDelta{Div,Sub} and InitGuardedChecksumSlot RETURN their
 * cell in EAX, which the decompile discarded; now captured into
 * iVar7/iVar10/iVar11/uVar8/uVar9/local_2480[0] and passed to the
 * following Peek. In the two Delta-Div branches at 0x4905ac/0x490655
 * the tree-node pointer (held in EDI/[esp+0x18] as node+0x25c and
 * node+0x15ac) is clobbered in C when iVar7 is overwritten by the Peek
 * result, so iVar12/iVar11 capture node+0x25c/node+0x15ac first -
 * exactly the values the binary keeps in edi and [esp+0x18].
 */
#include "ghidra_types.h"


/* WARNING: Function: __chkstk replaced with injection: alloca_probe */

undefined1 FUN_0048f300(int param_1)

{
  int *piVar1;
  undefined1 uVar2;
  uint uVar3;
  char cVar4;
  int *piVar5;
  int iVar6;
  int iVar7;
  undefined4 uVar8;
  undefined4 uVar9;
  int iVar10;
  int iVar11;
  int iVar12;
  undefined4 *unaff_FS_OFFSET;
  int local_249c;
  int local_2498;
  undefined1 local_2492;
  uint local_2490;
  int local_2480 [2];
  undefined1 local_2478 [8];
  undefined1 local_2470 [548];
  int iStack_245c;
  undefined1 local_224c [548];
  int iStack_2238;
  undefined1 local_2028 [548];
  int iStack_2014;
  undefined1 local_1e04 [548];
  int iStack_1df0;
  undefined1 local_1be0 [548];
  int iStack_1bcc;
  undefined1 local_19bc [548];
  undefined1 local_1798 [548];
  undefined1 local_1574 [548];
  undefined1 local_1350 [548];
  undefined1 local_112c [548];
  undefined1 local_f08 [548];
  undefined1 local_ce4 [548];
  undefined1 local_ac0 [548];
  undefined1 local_89c [548];
  undefined1 local_678 [548];
  undefined1 local_454 [548];
  undefined1 auStack_230 [20];
  int iStack_21c;
  undefined4 uStack_10;
  undefined4 uStack_c;
  undefined1 *puStack_8;
  undefined4 local_4;
  
  local_4 = 0xffffffff;
  uStack_c = *unaff_FS_OFFSET;
  puStack_8 = &LAB_005399d3;
  *unaff_FS_OFFSET = &uStack_c;
  uStack_10 = 0x48f31f;
  local_2492 = 0;
  local_2490 = 0;
  do {
    piVar5 = (int *)GetPlayerRecordBySlot(g_clientContext);
    if ((piVar5 != (int *)0x0) && (cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0xbff7), cVar4 != '\x01')) {
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x33c8));
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0x9ce4);
      if (cVar4 != '\0') {
        iVar6 = iVar6 / 2 + iVar6;
      }
      cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0x9ce7);
      if (cVar4 != '\0') {
        iVar6 = iVar6 + ((int)(iVar6 + (iVar6 >> 0x1f & 3U)) >> 2);
      }
      iVar7 = EncodeChecksumPairDiff(param_1 + 0x40,local_1350,piVar5 + 0x243);
      local_4 = 0;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar7 = PeekPacketChecksumState((void *)iVar7);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      local_4 = 0xffffffff;
      if ((*(int *)(local_1350 + 0x14)) != 0) {
        ScrambleChecksumGuardBytes(*(int *)(local_1350 + 0x14),&g_valueGuardKeyTable);
        TreeLowerBound(local_2480,&g_valueGuardMap);
      }
      local_2480[0] = EncodeChecksumPairDiff(param_1 + 0x264,local_1798,piVar5 + 0x2cc);
      local_4 = 1;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      local_2480[0] = PeekPacketChecksumState((void *)local_2480[0]);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      local_4 = 0xffffffff;
      if ((*(int *)(local_1798 + 0x14)) != 0) {
        ScrambleChecksumGuardBytes(*(int *)(local_1798 + 0x14),&g_valueGuardKeyTable);
        TreeLowerBound(local_2478,&g_valueGuardMap);
      }
      if (local_2480[0] * local_2480[0] + iVar7 * iVar7 < iVar6 * iVar6) {
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        local_2498 = PeekPacketChecksumState((void *)(piVar5 + 0x12db));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x40));
        iVar7 = PeekPacketChecksumState((void *)(piVar5 + 0x243));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        if (iVar7 < iVar6) {
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar6 = PeekPacketChecksumState((void *)(piVar5 + 0x715));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (iVar6 != 1) goto LAB_0048f583;
LAB_0048f5d0:
          local_2498 = -local_2498;
        }
        else {
LAB_0048f583:
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x40));
          iVar7 = PeekPacketChecksumState((void *)(piVar5 + 0x243));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (iVar6 < iVar7) {
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar6 = PeekPacketChecksumState((void *)(piVar5 + 0x715));
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            if (iVar6 == 0) goto LAB_0048f5d0;
          }
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar6 = PeekPacketChecksumState((void *)(piVar5 + 0x1948));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0x9ce4);
        if (cVar4 == '\x01') {
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33c2);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + -4;
          }
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33c5);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + 4;
          }
        }
        cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0x9ce7);
        if (cVar4 == '\x01') {
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33bf);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + -4;
          }
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33c2);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + 4;
          }
        }
        cVar4 = PeekPacketChecksumBool((byte *)piVar5 + 0x9cea);
        if (cVar4 == '\x01') {
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33bf);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + 4;
          }
          cVar4 = PeekPacketChecksumBool((byte *)param_1 + 0x33c5);
          if (cVar4 == '\x01') {
            iVar6 = iVar6 + -4;
          }
        }
        uVar8 = EncodeChecksumDeltaMul(g_clientContext + 0x15406c,local_f08,iVar6);
        local_4 = 2;
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        uVar9 = PeekPacketChecksumState((void *)&DAT_00796aa0);
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        uVar8 = EncodeChecksumDeltaDiv(uVar8,local_19bc,uVar9);
        SUBFIELD(local_4,0,undefined1) = 3;
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar6 = PeekPacketChecksumState((void *)uVar8);
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        local_4 = CONCAT31(SUBFIELD(local_4,1,undefined3),2);
        if ((*(int *)(local_19bc + 0x14)) != 0) {
          ScrambleChecksumGuardBytes(*(int *)(local_19bc + 0x14),&g_valueGuardKeyTable);
          TreeLowerBound(local_2478,&g_valueGuardMap);
        }
        local_4 = 0xffffffff;
        if ((*(int *)(local_f08 + 0x14)) != 0) {
          ScrambleChecksumGuardBytes(*(int *)(local_f08 + 0x14),&g_valueGuardKeyTable);
          TreeLowerBound(local_2478,&g_valueGuardMap);
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar7 = PeekPacketChecksumState((void *)(g_clientContext + 0x45354));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        if ((iVar7 != 3) && (piVar5[9] != 0xe)) {
          uVar8 = EncodeChecksumDeltaSub(piVar5 + 0x1bf5,local_ac0,iVar6);
          local_4 = 4;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar7 = PeekPacketChecksumState((void *)uVar8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_4 = 0xffffffff;
          ScrubChecksumGuard();
          if (iVar7 < 1) {
            QueueOutgoingPacketField(0);
          }
          else {
            QueueOutgoingPacketField(iVar7);
            iVar7 = 0;
          }
          AddToPacketChecksum(piVar5 + 0x1a5a, iVar7);
          iVar7 = piVar5[0x2c2a];
          piVar5[0x2c2a] = iVar7 + 1;
          uVar9 = EncodeChecksumDeltaSub(piVar5 + 0x2cc,local_678,iVar7 * 0xf);
          local_4 = 5;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          uVar8 = PeekPacketChecksumState((void *)uVar9);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          uVar9 = PeekPacketChecksumState((void *)(piVar5 + 0x243));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          FUN_00436860(uVar9,uVar8);
          local_4 = 0xffffffff;
          ScrubChecksumGuard();
          *(undefined1 *)(piVar5 + 0x2c2b) = *(undefined1 *)(param_1 + 0x3c);
          if (*(byte *)(param_1 + 0x3c) < 8) {
            if (*(char *)(*(byte *)(param_1 + 0x3c) + 0x4590c + g_clientContext) ==
                *(char *)((piVar5[2] & 7U) + 0x4590c + g_clientContext)) {
              *(int *)(g_clientContext + 0x5b83c + (uint)*(byte *)(param_1 + 0x3c) * 4) =
                   *(int *)(g_clientContext + 0x5b83c + (uint)*(byte *)(param_1 + 0x3c) * 4) + iVar6;
              *(undefined1 *)(piVar5 + 0x2c2d) = 1;
            }
            else {
              piVar1 = (int *)(g_clientContext + 0x5b81c + (uint)*(byte *)(param_1 + 0x3c) * 4);
              *piVar1 = *piVar1 + iVar6;
            }
          }
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar6 = PeekPacketChecksumState((void *)&DAT_00794e48);
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        if (local_2498 <= iVar6) {
          local_2492 = 1;
          piVar5[0x2ff7] = piVar5[0x2ff7] + local_2498 * -3 + 200;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar6 = PeekPacketChecksumState((void *)&DAT_00796aa0);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (iVar6 < piVar5[0x2ff7]) {
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar6 = PeekPacketChecksumState((void *)&DAT_00796aa0);
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar6 = piVar5[0x2ff7] / iVar6;
            cVar4 = CompareChecksumExceeds(param_1 + 0x40,piVar5 + 0x243);
            if (cVar4 == '\0') {
              uVar8 = EncodeChecksumDeltaSub(piVar5 + 0x2cc,local_224c,0x1e);
              local_4 = 0xb;
              uVar9 = EncodeChecksumDeltaDiv(piVar5 + 0x355,local_2028,2);
              SUBFIELD(local_4,0,undefined1) = 0xc;
              uVar9 = EncodeChecksumPairSum(piVar5 + 0x243,local_1e04,uVar9);
              SUBFIELD(local_4,0,undefined1) = 0xd;
              local_2480[0] = EncodeChecksumDeltaSub(piVar5 + 0x2cc,local_1be0,0x1e);
              SUBFIELD(local_4,0,undefined1) = 0xe;
              PeekChecksumStateUnderLock(uVar8);
              PeekChecksumStateUnderLock(uVar9);
              iVar7 = FindGroundHeightAtColumn();
              iVar10 = PeekChecksumStateUnderLock(local_2480[0]);
              SUBFIELD(local_4,0,undefined1) = 0xd;
              ScrubChecksumGuard();
              SUBFIELD(local_4,0,undefined1) = 0xc;
              ScrubChecksumGuard();
              local_4 = CONCAT31(SUBFIELD(local_4,1,undefined3),0xb);
              ScrubChecksumGuard();
              local_4 = 0xffffffff;
              ScrubChecksumGuard();
              if (iVar10 < iVar7) {
                AddToPacketChecksum(piVar5 + 0x243, iVar6);
              }
              uVar8 = EncodeChecksumDeltaAdd(piVar5 + 0x2cc,local_2470,0x14);
              local_4 = 0xf;
              iVar7 = (**(code **)(*piVar5 + 0x14))();
              iVar10 = PeekChecksumStateUnderLock(uVar8);
              local_4 = 0xffffffff;
              ScrubChecksumGuard();
              if (iVar10 < iVar7) {
                SubFromPacketChecksum(piVar5 + 0x243, iVar6);
              }
            }
            else {
              uVar8 = EncodeChecksumDeltaSub(piVar5 + 0x2cc,local_89c,0x1e);
              local_4 = 6;
              uVar9 = EncodeChecksumDeltaDiv(piVar5 + 0x355,local_112c,2);
              SUBFIELD(local_4,0,undefined1) = 7;
              uVar9 = EncodeChecksumPairDiff(piVar5 + 0x243,local_454,uVar9);
              SUBFIELD(local_4,0,undefined1) = 8;
              local_2480[0] = EncodeChecksumDeltaSub(piVar5 + 0x2cc,local_1574,0x1e);
              SUBFIELD(local_4,0,undefined1) = 9;
              PeekChecksumStateUnderLock(uVar8);
              PeekChecksumStateUnderLock(uVar9);
              iVar7 = FindGroundHeightAtColumn();
              iVar10 = PeekChecksumStateUnderLock(local_2480[0]);
              SUBFIELD(local_4,0,undefined1) = 8;
              ScrubChecksumGuard();
              SUBFIELD(local_4,0,undefined1) = 7;
              ScrubChecksumGuard();
              local_4 = CONCAT31(SUBFIELD(local_4,1,undefined3),6);
              ScrubChecksumGuard();
              local_4 = 0xffffffff;
              ScrubChecksumGuard();
              if (iVar10 < iVar7) {
                SubFromPacketChecksum(piVar5 + 0x243, iVar6);
                uVar8 = EncodeChecksumDeltaAdd(piVar5 + 0x2cc,local_ce4,0x14);
                local_4 = 10;
                iVar7 = (**(code **)(*piVar5 + 0x14))();
                iVar10 = PeekChecksumStateUnderLock(uVar8);
                local_4 = 0xffffffff;
                ScrubChecksumGuard();
                if (iVar10 < iVar7) {
                  AddToPacketChecksum(piVar5 + 0x243, iVar6);
                }
              }
            }
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar6 = PeekPacketChecksumState((void *)&DAT_00796aa0);
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            piVar5[0x2ff7] = piVar5[0x2ff7] % iVar6;
            EncodeChecksumState(piVar5 + 0x243);
            *(undefined1 *)(piVar5 + 0x2c2b) = *(undefined1 *)(param_1 + 0x3c);
          }
        }
      }
    }
    local_2490 = local_2490 + 1;
    if (7 < (int)local_2490) {
      iVar6 = param_1 + 0x33c8;
      iVar10 = InitGuardedChecksumSlot(iVar6,local_2470,iVar6);
      local_4 = 0x10;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar7 = PeekPacketChecksumState((void *)iVar10);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      local_4 = 0xffffffff;
      if (iStack_245c != 0) {
        iVar10 = iStack_245c << 4;
        local_2498 = 0x10;
        do {
          iVar11 = _rand();
          *(char *)(iVar10 + g_valueGuardKeyTable) = (char)iVar11;
          iVar10 = iVar10 + 1;
          local_2498 = local_2498 + -1;
        } while (local_2498 != 0);
        TreeLowerBound(local_2478,&g_valueGuardMap);
      }
      local_2490 = 0;
      do {
        iVar10 = *(int *)(*(int *)(&DAT_006a7f8c + g_clientContext) + 0x1c);
        uVar3 = *(uint *)(iVar10 + 4);
        while (uVar3 < 0x186a4) {
          if (uVar3 == 0x186a3) {
            piVar5 = *(int **)(iVar10 + 0x10);
            uVar3 = piVar5[2];
            if (uVar3 <= local_2490) goto LAB_0048fe95;
            break;
          }
          iVar10 = *(int *)(iVar10 + 0x1c);
          uVar3 = *(uint *)(iVar10 + 4);
        }
LAB_0049008d:
        local_2490 = local_2490 + 1;
      } while ((int)local_2490 < 0x40);
      iVar7 = InitGuardedChecksumSlot(iVar6,local_2470,iVar6);
      local_4 = 0x13;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar6 = PeekPacketChecksumState((void *)iVar7);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      local_4 = 0xffffffff;
      if (iStack_245c != 0) {
        iVar7 = iStack_245c << 4;
        iVar10 = 0x10;
        do {
          iVar11 = _rand();
          *(char *)(iVar7 + g_valueGuardKeyTable) = (char)iVar11;
          iVar7 = iVar7 + 1;
          iVar10 = iVar10 + -1;
        } while (iVar10 != 0);
        TreeLowerBound(local_2478,&g_valueGuardMap);
      }
      local_2490 = 0;
      do {
        iVar7 = *(int *)(*(int *)(&DAT_006a7f8c + g_clientContext) + 0x1c);
        uVar3 = *(uint *)(iVar7 + 4);
        while (uVar3 < 0x186a7) {
          if (uVar3 == 0x186a6) {
            iVar7 = *(int *)(iVar7 + 0x10);
            uVar3 = *(uint *)(iVar7 + 8);
            if (uVar3 <= local_2490) goto LAB_00490184;
            break;
          }
          iVar7 = *(int *)(iVar7 + 0x1c);
          uVar3 = *(uint *)(iVar7 + 4);
        }
LAB_00490747:
        local_2490 = local_2490 + 1;
        if (7 < (int)local_2490) {
          *unaff_FS_OFFSET = uStack_c;
          return local_2492;
        }
      } while( true );
    }
  } while( true );
LAB_0048fe95:
  if (uVar3 != local_2490) goto code_r0x0048fe97;
  iVar11 = EncodeChecksumPairDiff(param_1 + 0x40,local_224c,piVar5 + 0x10);
  local_4 = 0x11;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar10 = PeekPacketChecksumState((void *)iVar11);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  if (iStack_2238 != 0) {
    iVar11 = iStack_2238 << 4;
    local_2498 = 0x10;
    do {
      iVar12 = _rand();
      *(char *)(iVar11 + g_valueGuardKeyTable) = (char)iVar12;
      iVar11 = iVar11 + 1;
      local_2498 = local_2498 + -1;
    } while (local_2498 != 0);
    TreeLowerBound(local_2478,&g_valueGuardMap);
  }
  iVar11 = EncodeChecksumPairDiff(param_1 + 0x264,local_2028,piVar5 + 0x99);
  local_4 = 0x12;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar11 = PeekPacketChecksumState((void *)iVar11);
  local_2480[0] = iVar11;
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  if (iStack_2014 != 0) {
    iVar11 = iStack_2014 << 4;
    local_2498 = 0x10;
    do {
      iVar12 = _rand();
      *(char *)(iVar11 + g_valueGuardKeyTable) = (char)iVar12;
      iVar11 = iVar11 + 1;
      local_2498 = local_2498 + -1;
    } while (local_2498 != 0);
    TreeLowerBound(local_2478,&g_valueGuardMap);
    iVar11 = local_2480[0];
  }
  if (iVar10 * iVar10 + iVar11 * iVar11 < iVar7) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar10 = PeekPacketChecksumState((void *)(param_1 + 0x40));
    iVar11 = PeekPacketChecksumState((void *)(piVar5 + 0x10));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    if ((iVar10 <= iVar11) || (piVar5[0xfea] != 0)) {
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      PeekPacketChecksumState((void *)(param_1 + 0x40));
      PeekPacketChecksumState((void *)(piVar5 + 0x10));
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    }
    (**(code **)(*piVar5 + 0x20))();
  }
  goto LAB_0049008d;
code_r0x0048fe97:
  piVar5 = (int *)piVar5[4];
  uVar3 = piVar5[2];
  if (local_2490 < uVar3) goto LAB_0049008d;
  goto LAB_0048fe95;
LAB_00490184:
  if (uVar3 != local_2490) goto code_r0x00490186;
  iVar10 = EncodeChecksumPairDiff(param_1 + 0x40,local_224c,iVar7 + 0x25c);
  local_4 = 0x14;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_2480[0] = PeekPacketChecksumState((void *)iVar10);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  if (iStack_2238 != 0) {
    iVar10 = iStack_2238 << 4;
    local_2498 = 0x10;
    do {
      iVar11 = _rand();
      *(char *)(iVar10 + g_valueGuardKeyTable) = (char)iVar11;
      iVar10 = iVar10 + 1;
      local_2498 = local_2498 + -1;
    } while (local_2498 != 0);
    TreeLowerBound(local_2478,&g_valueGuardMap);
  }
  iVar11 = EncodeChecksumPairDiff(param_1 + 0x264,local_2028,iVar7 + 0x480);
  local_4 = 0x15;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar10 = PeekPacketChecksumState((void *)iVar11);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  if (iStack_2014 != 0) {
    iVar11 = iStack_2014 << 4;
    local_2498 = 0x10;
    do {
      iVar12 = _rand();
      *(char *)(iVar11 + g_valueGuardKeyTable) = (char)iVar12;
      iVar11 = iVar11 + 1;
      local_2498 = local_2498 + -1;
    } while (local_2498 != 0);
    TreeLowerBound(local_2478,&g_valueGuardMap);
  }
  if (local_2480[0] * local_2480[0] + iVar10 * iVar10 < iVar6) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar11 = PeekPacketChecksumState((void *)(iVar7 + 0xaf0));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar2 = *(undefined1 *)(param_1 + 0x3c);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EncodeOutgoingPacketField((void *)(iVar7 + 0xd14),uVar2);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar10 = PeekPacketChecksumState((void *)(g_clientContext + 0x15406c));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar12 = PeekPacketChecksumState((void *)(iVar7 + 0x8c8));
    EncodeOutgoingPacketField((void *)(iVar7 + 0x8c8),iVar12 + iVar10);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar10 = *(int *)(iVar7 + 0xaec);
    *(int *)(iVar7 + 0xaec) = iVar10 + 1;
    iVar10 = EncodeChecksumDeltaSub(iVar7 + 0x480,local_1e04,iVar10 * 0xf);
    local_4 = 0x16;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)(g_clientContext + 0x15406c));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar8 = PeekPacketChecksumState((void *)iVar10);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    local_2480[0] = PeekPacketChecksumState((void *)(iVar7 + 0x25c));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    FUN_00436860(local_2480[0],uVar8);
    local_4 = 0xffffffff;
    if (iStack_1df0 != 0) {
      iVar10 = iStack_1df0 << 4;
      local_2498 = 0x10;
      do {
        iVar12 = _rand();
        *(char *)(iVar10 + g_valueGuardKeyTable) = (char)iVar12;
        iVar10 = iVar10 + 1;
        local_2498 = local_2498 + -1;
      } while (local_2498 != 0);
      TreeLowerBound(local_2478,&g_valueGuardMap);
    }
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar10 = PeekPacketChecksumState((void *)&DAT_00794e48);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    if (iVar11 <= iVar10) {
      local_2492 = 1;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar10 = PeekPacketChecksumState((void *)(iVar7 + 0x15ac));
      EncodeOutgoingPacketField((void *)(iVar7 + 0x15ac),iVar10 + (100 - iVar11) * 2);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar10 = PeekPacketChecksumState((void *)&DAT_00796aa0);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar11 = PeekPacketChecksumState((void *)(iVar7 + 0x15ac));
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      if (iVar10 < iVar11) {
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar10 = PeekPacketChecksumState((void *)(param_1 + 0x40));
        iVar11 = PeekPacketChecksumState((void *)(iVar7 + 0x25c));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        uVar8 = PeekPacketChecksumState((void *)&DAT_00796aa0);
        if (iVar10 < iVar11) {
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar10 = EncodeChecksumDeltaDiv(iVar7 + 0x15ac,local_1be0,uVar8);
          local_4 = 0x17;
          iVar12 = iVar7 + 0x25c;
          iVar11 = iVar7 + 0x15ac;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar7 = PeekPacketChecksumState((void *)iVar12);
          iVar10 = PeekPacketChecksumState((void *)iVar10);
          EncodeOutgoingPacketField((void *)iVar12,iVar10 + iVar7);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_4 = 0xffffffff;
          if (iStack_1bcc != 0) {
            iVar7 = iStack_1bcc << 4;
            local_249c = 0x10;
            do {
              iVar10 = _rand();
              *(char *)(iVar7 + g_valueGuardKeyTable) = (char)iVar10;
              iVar7 = iVar7 + 1;
              local_249c = local_249c + -1;
            } while (local_249c != 0);
LAB_004906f3:
            TreeLowerBound(local_2478,&g_valueGuardMap);
          }
        }
        else {
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar10 = EncodeChecksumDeltaDiv(iVar7 + 0x15ac,auStack_230,uVar8);
          local_4 = 0x18;
          iVar12 = iVar7 + 0x25c;
          iVar11 = iVar7 + 0x15ac;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar7 = PeekPacketChecksumState((void *)iVar12);
          iVar10 = PeekPacketChecksumState((void *)iVar10);
          EncodeOutgoingPacketField((void *)iVar12,iVar7 - iVar10);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_4 = 0xffffffff;
          if (iStack_21c != 0) {
            iVar7 = iStack_21c << 4;
            local_249c = 0x10;
            do {
              iVar10 = _rand();
              *(char *)(iVar7 + g_valueGuardKeyTable) = (char)iVar10;
              iVar7 = iVar7 + 1;
              local_249c = local_249c + -1;
            } while (local_249c != 0);
            goto LAB_004906f3;
          }
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar7 = PeekPacketChecksumState((void *)&DAT_00796aa0);
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar10 = PeekPacketChecksumState((void *)iVar11);
        EncodeOutgoingPacketField((void *)iVar11,iVar10 % iVar7);
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      }
    }
  }
  goto LAB_00490747;
code_r0x00490186:
  iVar7 = *(int *)(iVar7 + 0x10);
  uVar3 = *(uint *)(iVar7 + 8);
  if (local_2490 < uVar3) goto LAB_00490747;
  goto LAB_00490184;
}

