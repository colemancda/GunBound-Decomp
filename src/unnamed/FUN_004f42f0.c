/* FUN_004f42f0 - 0x004f42f0 in the original binary.
 *
 * No confirmed real name/purpose. Raw/near-verbatim port of Ghidra's
 * decompiler output, not hand-verified. See src/README.md's "Raw/
 * verbatim ports" section for status.
 */
#include "ghidra_types.h"


void FUN_004f42f0(int param_1)

{
  int *piVar1;
  undefined4 local_c;
  undefined1 *puStack_8;
  undefined4 local_4;
  /* Windows SEH __try/__except frame setup stripped - handler body
   * (LAB_00537748) wasn't included in this function's own decompile.
   * Same rationale as entry/InitGame.c - see src/README.md. */
  local_4 = 0;
  /* Container passed in EBX by the original (`mov ebx,[esp+0x14]` immediately
   * before the call, which esp-tracking through the SEH prologue resolves to
   * this function's own param_1) - see src/unnamed/FUN_004f46b0.c's header. */
  FUN_004f46b0(param_1);
  local_4 = 0xffffffff;
  piVar1 = *(int **)(param_1 + 0x110);
  if (piVar1 != (int *)0x0) {
    (**(code **)(*piVar1 + 8))(piVar1);
  }
  *(undefined4 *)(param_1 + 0x110) = 0;
  return;
}

