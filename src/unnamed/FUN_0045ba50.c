 ...
 * cell at all 24 argless PeekPacketChecksumState() calls.  The
 * worklist's MISMATCH (24 orig : 30 C at generation time) is stale -
 * the tree today has exactly 24 argless sites, and with no gotos the
 * address-order zip holds 1:1 (verified against a full disasm of
 * 0x45ba50-0x45c6f0 via tools/guard_cell_resolve.py plus a
 * frame-slot-writer scan).
 *
 * Eleven cells are chained helper returns the decompile discarded, all
 * captured in a new uVar11 (plus uVar12 where two are live at once):
 * the three FindGroundHeightAtColumn rounds each Peek the return of
 * their opening EncodeChecksumPairDiff (0x45baa7/0x45bbbc/0x45bd7c -
 * the third round's second operand, EncodeChecksumPairSum's return, was
 * already captured into local_ad0[0] and is reused as-is), and every
 * later DeltaAdd/DeltaSub round re-reads its helper's dest through a
 * frame[0x14] spill - each writer verified as `call <helper> /
 * mov [esp+0x14],eax` (0x45bfe3, 0x45c0ef, 0x45c1b9, 0x45c280-chain,
 * 0x45c319, 0x45c471, 0x45c550, 0x45c615).
 *
 * The rest are param_1 cells beside already-fixed Encodes, including
 * the dx/dy pair (+0x119c, +0xd54) feeding CalculateAngleFromDelta at
 * ~line 158 - the same offsets the three PairDiff rounds combine, which
 * reads as position-delta -> angle for this projectile's steering.
 */
...

{
  ...
  *unaff_FS_OFFSET = &local_c;
  ...
  local_4 = 2;
  uVar11 = EncodeChecksumPairDiff(param_1 + 0xb30,local_89c,uVar2);
  ...
  local_4 = 6;
  uVar11 = EncodeChecksumPairDiff(param_1 + 0xb30,local_678,uVar2);
