/* StringMap_Find_1c - 0x004ff130 in the original binary.
 *
 * The string-keyed map find over 0x1c-byte records: hash the key, take it modulo
 * the bucket count, then walk the chain at bucket[i] comparing the node's
 * hash at +0x18 and then the key itself.  Returns the node, and reports the
 * hash, the bucket index and the predecessor node through its three
 * out-parameters.  Its insert half is StringMap_Insert_1c.
 *
