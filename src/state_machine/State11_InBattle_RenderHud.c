 ...
 * cell at all 48 argless PeekPacketChecksumState() calls.  The C is in
 * address order here (its own LAB_004c8c40 / LAB_004c8d54 / LAB_004c8edb
 * labels line up with the guard calls at 0x4c8c55 / 0x4c8d69 / 0x4c8ef0),
 * so the 48 C sites map 1:1 onto the 48 guard calls between 0x4c8c55 and
 * 0x4c9a5a.
 *
 * The body is three identical 14-call groups (the three HUD gauges),
 * differing only in their arena base offsets:
 *   group 1 (C312..C357)  0x1a8a50 / 0x1a7930
 *   group 2 (C378..C423)  0x1a45d0 / 0x1a34b0
 *   group 3 (C444..C489)  0x1a6810 / 0x1a56f0
 * Each group reads a selector from g_clientContext + 0xeb854, uses it
 * (signed % 0x400, * 0x224) to pick a cell in the + 0x62854 array, and
 * that cell's value (& 7) is the slot index for five arena reads of the
 * form
 *   (<sel> * 0xb + *(byte *)(ctx + 0x475c4)) * 0x7d28 + ctx
 *       + (slot & 7) * 0x224 + <base>
 * where <sel> is a fresh peek of g_clientContext + 0x475c8 taken
 * immediately before each one.  Ghidra DISCARDED every one of those
 * intermediate results (they were bare call statements), so they are
 * captured here into two new locals, iGuardSel and iGuardSlot; a third,
 * iGuardA, holds the g_clientContext + 0x473a0 divisor the head block
 * needs.
 *
 * The head (C297..C308) is the same idea with a different index formula:
 * 0x4c8efe's `dec ebx; jns` is the `-1 < iVar6 + -1` test, EBP is zeroed
 * at 0x4c8f01 on the not-taken path and set to <peek> - 1 on the taken
 * one, and 0x4c8f4c-0x4c8f70 divides it by the 0x473a0 peek, takes the
 * signed remainder mod 8 twice around a +1, and scales by 0x224 into the
 * + 0x595d8 array.  Hence the added `iGuardSel = 0;` before the if.
 *
 * RENDER-CHAIN ARG RECOVERY (2026-09-02): recovered the dropped
 * (frame,x,y,outerKey) at all 37 render blocks - the 18 argless
 * FindSpriteFrame() gates (all FindSpriteFrame(&g_spriteRegistry,
 * 0x3f3,1), EAX=0xea0e18/EDX/ESI at 0x4c9f3b..0x4ca57d) plus the
 * inlined tree-walk blocks that call BlitSprite16bpp/BlitSpriteClipped
 * directly.  Method: disasm 0x4c8890-0x4cad5c, pair blocks by the
 * hoisted while-loops' own LAB_ addresses (the C is NOT in VA order
 * here) and by the x/y expressions already present as the two stale
 * args.  Keys per region: 0x34 (HUD head), 0x2328 (nametag rows),
 * 0x1b58 (the 0x33e-0x340 banner trio + the two 0xde/0x1e0 blocks,
 * frames 0xe/0xae, already fixed earlier), 0x3f3 (gauge icons, frame
 * ebx=1 throughout), 0x2329 (frame = the 0x62155 flag, uVar10).
 * FRAME at the walk blocks is the walk's own search key (the register
 * compared against node+8; the clipped calls already carried it).
 * MSVC's shared clipped tails (LAB_004c9eeb/004ca0d3/004ca3ad/
 * 004ca58c/004ca1ce/004ca724, reached by per-branch `mov ecx,x` +
 * jmp) were de-shared: each goto site got its own else-clause
 * BlitSpriteClipped with that branch's x, per the disasm.
 */
...
LAB_004c897e:
    if (uVar11 == uVar10) {
      if (*(char *)(iVar5 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar10,0x14,pcStack_9c + iVar7,0x34);
      }
      else {
        BlitSpriteClipped(uVar10,0x14,pcStack_9c + iVar7,0x34);
      }
      break;
    }
  }
...
LAB_004c8a87:
  ...
  /* RE-SLOT + dropped EAX (objdump @0x4c8a90-0x4c8aa3): x/ECX = edi = 0x20
   * (`mov edi,0x20` at 0x4c89b4, the same left-margin the BlitRLESprite
   * below reads via its own `mov ecx,edi`), y = ebx = pcVar4, colour =
   * ecx = uVar8, string = EAX = [esp+0x2c] - the very slot that call
   * reloads (as [esp+0x34], two pushes deeper) for its rleData. */
  DrawFontString(0x20,pcVar4,uVar8,pcStack_94);
  ...
  if ((*pbStack_90 < 2) || (*pbStack_90 == 7)) {
    ...
    /* RE-SLOT + dropped EAX (objdump @0x4c8adc-0x4c8ae8): x/ECX =
     * [esp+0x28] + edi = 0x20 + iLen94*6 - the same slot+register the
     * BlitRLESprite below rebuilds as `lea ecx,[edi+edx]`; y = ebx =
     * pcVar4; colour = esi = uStack_b0; string = EAX = [esp+0x24] =
     * pcStack_9c, the slot that call reloads as its rleData. */
    DrawFontString(0x20 + iLen94 * 6,pcVar4,uStack_b0,pcStack_9c);
    ...
    /* RE-SLOT + dropped EAX (objdump @0x4c8b17-0x4c8b3a): x/ECX = ebx =
     * 0x21 + iLen94*6 + iLen9c*6 and y = edi = pcStack_9c + iVar6, both
     * held in the same registers by the BlitRLESprite below (`mov
     * ecx,ebx` / `push edi`); colour = esi = uStack_b0; string = EAX =
     * [esp+0x20], the slot that call reloads as g_clientContext+0x58c54. */
    DrawFontString(0x21 + iLen94 * 6 + iLen9c * 6,pcStack_9c + iVar6,uStack_b0,
                   (char *)(g_clientContext + 0x58c54));
...
LAB_004c8edb:
  ...
  if (-1 < iVar6 + -1) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iGuardSel = -1 + PeekPacketChecksumState((void *)(g_clientContext + 0xeba98));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iGuardA = PeekPacketChecksumState((void *)(g_clientContext + 0x473a0));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar6 = PeekPacketChecksumState((void *)(((iGuardSel / iGuardA) % 8 + 1) % 8 * 0x224 + g_clientContext + 0x595d8));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  if (iVar6 == 0) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iGuardSel = PeekPacketChecksumState((void *)(g_clientContext + 0xeb854));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iGuardSlot = PeekPacketChecksumState((void *)(iGuardSel % 0x400 * 0x224 + g_clientContext + 0x62854));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iGuardSel = PeekPacketChecksumState((void *)(g_clientContext + 0x475c8));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar6 = PeekPacketChecksumState((void *)((iGuardSel * 0xb + *(byte *)(g_clientContext + 0x475c4)) * 0x7d28 + g_clientContext +
                                                     (iGuardSlot & 7) * 0x224 + 0x1a8a50));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iGuardSel = PeekPacketChecksumState((void *)(g_clientContext + 0x475c8));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar7 = PeekPacketChecksumState((void *)((iGuardSel * 0xb + *(byte *)(g_clientContext + 0x475c4)) * 0x7d28 + g_clientContext +
                                                     (iGuardSlot & 7) * 0x224 + 0x1a7930));
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    if (iVar6 - iVar7 < 1) {
      uStack_a4 = 1;
    }
...
LAB_004c8ce0:
    if (uVar11 == uVar10) {
      if (*(char *)(iVar6 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar10,(piVar12[-0x100] - *(int *)(&g_nCameraX + g_clientContext)) + 400,
                        0x24,0x2328);
      }
      else {
        BlitSpriteClipped(uVar10,(piVar12[-0x100] - *(int *)(&g_nCameraX + g_clientContext)) + 400,
                          0x24,0x2328);
      }
      break;
    }
...
LAB_004c8df4:
    if (uVar11 == uVar10) {
      if (*(char *)(iVar6 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar10,(piVar12[-0x80] - *(int *)(&g_nCameraX + g_clientContext)) + 400,
                        0x24,0x2328);
      }
      else {
        BlitSpriteClipped(uVar10,(piVar12[-0x80] - *(int *)(&g_nCameraX + g_clientContext)) + 400,
                          0x24,0x2328);
      }
      break;
    }
...
LAB_004c8e28:
  ...
  /* RE-SLOT + dropped EAX (objdump @0x4c8e9d-0x4c8eb5): x/ECX = ebx, the
   * identical centred nametag cursor the BlitRLESprite directly above
   * passes (`mov ecx,ebx` at both 0x4c8e96 and 0x4c8eb3, ebx untouched in
   * between); y = 0x2d; colour = 0; string = EAX =
   * `imul edx,[ebp],0xd / lea eax,[edx+eax+0x50196]` = pcVar3, the name
   * string sprintf'd and strlen'd just above. */
  DrawFontString(piVar12[-0x80] - iLen9c * 3 - *(int *)(&g_nCameraX + g_clientContext) + 400,
                 0x2d,0,pcVar3);
  iStack_a0 = iStack_a0 + 1;
  piVar12 = piVar12 + 1;
...
LAB_004c9ad7:
    if (uVar2 == uVar11) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar11,((uVar10 % uStack_a4 + iVar6) - *(int *)(&g_nCameraX + g_clientContext))
                        + 400,0x24,0x1b58);
      }
      else {
        BlitSpriteClipped(uVar11,((uVar10 % uStack_a4 + iVar6) - *(int *)(&g_nCameraX + g_clientContext))
                          + 400,0x24,0x1b58);
      }
      break;
    }
...
LAB_004c9b12:
  ...
  iLen9c = (int)(pcVar4 - pcVar3) - 1;
  uVar14 = 0;
  /* RE-SLOT + dropped EAX (objdump @0x4c9b4a-0x4c9b64): x/ECX =
   * esi - g_nCameraX - edi + 0x190, the identical centred cursor the
   * BlitRLESprite below rebuilds; y = 0x28; colour = the `push 0` the
   * compiler hoisted above the GetLocalizedString call (0x4c9b33, left on
   * the stack because GetLocalizedString is `ret 8`); string = EAX = that
   * call's return value, untouched between 0x4c9b45 and the call.  Ghidra
   * discarded the result, leaving a bare call statement - folded back into
   * the argument here so the call count is unchanged. */
  DrawFontString((int)(uVar10 % uStack_a4 + iVar6) - *(int *)(&g_nCameraX + g_clientContext) -
                 iLen9c * 3 + 400,0x28,uVar14,
                 (char *)GetLocalizedString(&g_localizedStringTable,0x33e));
  uVar14 = 0xffff;
...
LAB_004c9714:
  ...
  iLen9c = (int)(pcVar4 - pcVar3) - 1;
  uVar14 = 0;
  /* RE-SLOT + dropped EAX (objdump @0x4c974c-0x4c9766) - identical shape
   * to the 0x33e block above: x/ECX = esi - g_nCameraX - edi + 0x190,
   * y = 0x28, colour = the `push 0` hoisted at 0x4c9735 above the
   * `ret 8` GetLocalizedString call, string = EAX = that call's result. */
  DrawFontString((int)(uVar10 % uStack_a4 + iVar6) - *(int *)(&g_nCameraX + g_clientContext) -
                 iLen9c * 3 + 400,0x28,uVar14,
                 (char *)GetLocalizedString(&g_localizedStringTable,0x33f));
  uVar14 = 0xffff;
...
LAB_004c9316:
  ...
  iLen9c = (int)(pcVar4 - pcVar3) - 1;
  uVar14 = 0;
  /* RE-SLOT + dropped EAX (objdump @0x4c934e-0x4c9368) - identical shape
   * to the 0x33e block above: x/ECX = esi - g_nCameraX - edi + 0x190,
   * y = 0x28, colour = the `push 0` hoisted at 0x4c9337 above the
   * `ret 8` GetLocalizedString call, string = EAX = that call's result. */
  DrawFontString((int)(uVar10 % uStack_a4 + iVar6) - *(int *)(&g_nCameraX + g_clientContext) -
                 iLen9c * 3 + 400,0x28,uVar14,
                 (char *)GetLocalizedString(&g_localizedStringTable,0x340));
  uVar14 = 0xffff;
...
LAB_004c9ba8:
  ...
  FlushCompositorLayer(3,(int)&g_spriteDrawBatchPool);
...
LAB_004c9d76:
    if (bVar13) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(0,8,0x2d,0x3f3);
      }
      else {
        BlitSpriteClipped(0,8,0x2d,0x3f3);
      }
      break;
    }
  }
...
LAB_004c9ebe:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x71,0x72,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x71,0x72,0x3f3);
      }
      break;
    }
  }
...
LAB_004c9e56:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xc6,0x72,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xc6,0x72,0x3f3);
      }
      break;
    }
  }
...
LAB_004c9ddb:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x1d,0x72,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x1d,0x72,0x3f3);
      }
      break;
    }
  }
LAB_004c9ef9:
  ...
  case 2:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x27,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x27,0xa8,0x3f3);
      }
    }
    break;
  case 3:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x4f,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x4f,0xa8,0x3f3);
      }
    }
    break;
  case 4:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x77,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x77,0xa8,0x3f3);
      }
    }
    break;
  case 5:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x9f,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x9f,0xa8,0x3f3);
      }
    }
    break;
  case 6:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,199,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,199,0xa8,0x3f3);
      }
    }
    break;
  case 7:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xef,0xa8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xef,0xa8,0x3f3);
      }
    }
  }
...
LAB_004ca1dc:
  ...
  case 0:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x59,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x59,0xf8,0x3f3);
      }
    }
    break;
  case 0x50:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x77,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x77,0xf8,0x3f3);
      }
    }
    break;
  case 0x5a:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x95,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x95,0xf8,0x3f3);
      }
    }
    break;
  case 0x5f:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xb3,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xb3,0xf8,0x3f3);
      }
    }
    break;
  case 0x62:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xd1,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xd1,0xf8,0x3f3);
      }
    }
    break;
  case 100:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xef,0xf8,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xef,0xf8,0x3f3);
      }
    }
  }
  ...
  case 0:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x59,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x59,0x112,0x3f3);
      }
    }
    break;
  case 0x50:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x77,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x77,0x112,0x3f3);
      }
    }
    break;
  case 0x5a:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x95,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x95,0x112,0x3f3);
      }
    }
    break;
  case 0x5f:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xb3,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xb3,0x112,0x3f3);
      }
    }
    break;
  case 0x62:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xd1,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xd1,0x112,0x3f3);
      }
    }
    break;
  case 100:
    if ((g_screenSurface != 0) && (iVar7 = FindSpriteFrame((int)&g_spriteRegistry,0x3f3,1), iVar7 != 0)) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xef,0x112,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xef,0x112,0x3f3);
      }
    }
  }
...
LAB_004ca19e:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x23,0xda,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x23,0xda,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca133:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xd6,0xda,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xd6,0xda,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca6f1:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xc6,0x144,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xc6,0x144,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca680:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x71,0x144,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x71,0x144,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca5fd:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x1d,0x144,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x1d,0x144,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca813:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0xdb,0x176,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0xdb,0x176,0x3f3);
      }
      break;
    }
  }
...
LAB_004ca787:
    if (uVar10 == 1) {
      if (*(char *)(iVar7 + 0x18) == '\x01') {
        BlitSprite16bpp(1,0x23,0x176,0x3f3);
      }
      else {
        BlitSpriteClipped(1,0x23,0x176,0x3f3);
      }
      break;
    }
  }
...
LAB_004caa9a:
    if (uVar11 == uVar10) {
      if (*(char *)(iVar6 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar10,0x18,0x30,0x2329);
      }
      else {
        BlitSpriteClipped(uVar10,0x18,0x30,0x2329);
      }
      break;
    }
  }
...
LAB_004cac35:
    if (uVar11 == uVar10) {
      if (*(char *)(iVar6 + 0x18) == '\x01') {
        BlitSprite16bpp(uVar10,0x193,0x30,0x2329);
      }
      else {
        BlitSpriteClipped(uVar10,0x193,0x30,0x2329);
      }
      break;
    }
  }
