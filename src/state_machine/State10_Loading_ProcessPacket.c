/* State10_Loading_ProcessPacket - 0x0043e280 in the original binary.
 *
 * Renamed with confidence: vtable_State10_Loading's slot 1 - dispatches
 * on `param_2` as a packet opcode (`0x3fff`, `0x2001`, `0x3233`,
 * `0x4410`), matching the same slot-1 "ProcessPacket" role already
 * confirmed for State11 (State11_InBattle_ProcessPacket, same slot
 * number, same param_2-as-opcode shape).
 *
...
void State10_Loading_ProcessPacket(undefined4 param_1,int param_2,short *param_3)
