/* DetonateShot2_Bullet7 - 0x00495e80 in the original binary.
 *
 * RENAMED (2026-08-16, from FUN_00495e80): CProjectile virtual slot 6
 * (DetonateProjectile, base impl 0x4572b0) override for mobile type 6's
 * shot 2 (weapon index 1, texture bullet7s); vtable 0x5563d0, ctor
 * InitShot2_Bullet7 reached only through SpawnPrimaryShot's case 6. Type/role
 * CONFIRMED via vtable geometry (tools/projectile_class_map.py); the
 * mobile NAME is not confirmable (no mobile-type->name map in the repo).
 *
 * Body is a raw/near-verbatim Ghidra port, not hand-verified. See
 * src/README.md's "Raw/verbatim ports" section for status.
 *
 * DROPPED-ARG FIX (2026-08-11): CValueGuard call-site sweep
 * (tools/guard_worklist.json) - the 76 argless PeekPacketChecksumState()
 * and 24 cell-less EncodeOutgoingPacketField() calls now pass their
 * guard-cell pointers (EAX / EDI at the original calls), recovered by
 * per-site landmark matching against the original disasm
 * (0x495e80-0x497728) - NOT order-zipped: the worklist was flagged
 * MISMATCH because its 90-peek/26-encode orig lists include 2+1 sites
 * of the UNPORTED neighbor function at 0x497730 (address-containment
 * bleed); this function's true counts are exactly 76/24. Notes:
 *   - most cells are param_1 int-word offsets (lea [esi+4*N]); the
 *     [esp+0x28]/[esp+0x30] slot cells are the C's own local_d00/
 *     local_cf8 pointer copies (stores at 0x495ebc/0x495ef5).
 *   - the argless Encodes had also dropped their VALUE pushes: each is
 *     the preceding Peek's return (captured into uVar5/iVar4), the
 *     stanza sites at 0x496001-0x49609e add the stanza-head peek
 *     (local_d14), and 0x49623f/0x496269 use ((angle-peek)<<8)+peek
 *     where the first term is FloatToInt64's 0x4961de return (was a
 *     discarded call at old L182 - now captured in iVar4).
 *   - chained-return cells (helper returns its cell in EAX, decompile
 *     discarded it): EncodeChecksumDeltaMod 0x4960f9 -> peek 0x496111,
 *     EncodeChecksumPairDiff 0x496341 -> peek 0x49635c,
 *     EncodeChecksumDeltaShr 0x4970c5/0x497142/0x49751b/0x49753c ->
 *     peeks 0x4970e0/0x49715d/0x497554/0x49756d; captured in uVar5/6.
 *   - the two Encode(unaff_EBP)/Encode(puVar33) value args were Ghidra
 *     artifacts; the real values are the re-encode reads of the same
 *     cells' earlier peeks: [esp+0x2c]=local_d04 (0x49628e store) and
 *     [esp+0x24]=local_d10[0] (0x4962ab store), cells matching their
 *     peek sites 0x496284/0x4962a1 exactly.
 *   - global cells: &DAT_00e9bed8/&DAT_00e55ab8/&DAT_007949c8
 *     (immediates), g_clientContext+0x5b85c (mov edi,[0x5b3484];
 *     add edi,0x5b85c @0x496e6b) and +0xebcbc (lea @0x496699).
 *   - the branch pair around FUN_004e4fe0 uses TWO STACK cells at
 *     frame slots +0x44/+0x268 (0x224 apart = the guard-cell stride);
 *     Ghidra scatter-names them &stack0xfffff2e4 / auStack_af8 (its
 *     own names for those slots at the kept Sync/PeekUnderLock args) -
 *     passed as such. CAVEAT for flip time: like the pre-existing
 *     Sync/PeekChecksumStateUnderLock args, these locals are scattered
 *     1/4-byte decls, not a coalesced 0x224 frame cell.
 *   - ESI holds param_1 until 0x497050 (the terminal _rand block), so
 *     all esi-based cells on the edited paths are param_1-relative;
 *     EDI/EBX cells back-tracked per site incl. across callee-saved
 *     call spans (e.g. lea edi,[esi+0xf54] @0x4964f1 covering the four
 *     peeks 0x4964f9-0x496660).
 */
#include "ghidra_types.h"


void __fastcall DetonateShot2_Bullet7(int *param_1)

{
  int iCraterX;
  int iCraterY;
  /* Ghidra artifact: raw stack reference the decompiler could not
   * map to a named local; declared so the raw port parses. */
  undefined stack0xfffff2c8;
  undefined stack0xfffff2d8;
  undefined stack0xfffff2dc;
  undefined stack0xfffff2e0;
  undefined stack0xfffff2e4;
  byte bVar1;
  char cVar2;
  char cVar3;
  int iVar4;
  undefined4 uVar5;
  undefined4 uVar6;
  char *pcVar7;
  int iVar8;
  int iVar9;
  undefined4 uVar10;
  undefined4 uVar11;
  undefined4 uVar12;
  int iVar13;
  uint uVar14;
  int iVar15;
  int iVar16;
  byte bVar17;
  code *pcVar18;
  undefined4 unaff_EBP;
  byte *pbVar19;
  undefined4 *unaff_FS_OFFSET;
  bool bVar20;
  undefined4 uVar21;
  undefined4 uVar22;
  undefined4 uVar23;
  char cVar24;
  undefined *puVar25;
  undefined4 *puVar26;
  undefined4 *puVar27;
  undefined *puVar28;
  int *piVar29;
  int *piVar30;
  undefined *puVar31;
  undefined *puVar32;
  undefined *puVar33;
  int iImpactX;
  undefined4 uImpactPower;
  undefined1 auStack_d18 [3];
  char local_d15;
  int local_d14;
  undefined4 local_d10 [0x89]; /* CValueGuard cell: 0x89*4 = 0x224 bytes */
  undefined4 uStack_d08;
  int local_d04;
  int *local_d00;
  int *local_cf8;
  undefined1 auStack_cf4 [12];
  undefined *puStack_ce8;
  int local_ce4;
  int local_cd0;
  undefined1 uStack_afc;
  undefined1 auStack_af8 [4];
  undefined1 auStack_af4 [16];
  undefined4 uStack_ae4;
  int local_aac;
  undefined1 uStack_8d8;
  undefined1 auStack_8d0 [532];
  undefined1 auStack_6bc [20];
  int iStack_6a8;
  undefined1 auStack_488 [532];
  undefined1 auStack_274 [20];
  int iStack_260;
  undefined4 uStack_50;
  undefined4 uStack_48;
  undefined4 uStack_3c;
  undefined4 uStack_38;
  undefined4 uStack_34;
  undefined4 uStack_30;
  undefined4 uStack_1c;
  undefined4 uStack_18;
  undefined4 uStack_14;
  undefined4 uStack_c;
  undefined1 *puStack_8;
  int local_4;
  
  local_4 = 0xffffffff;
  puStack_8 = &LAB_0053e9f5;
  uStack_c = *unaff_FS_OFFSET;
  *unaff_FS_OFFSET = &uStack_c;
  local_d10[0] = 0;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d00 = param_1 + 0xfe9;
  iVar4 = PeekPacketChecksumState((void *)local_d00);
  pcVar18 = (code *)LeaveCriticalSection;
  local_d15 = iVar4 == -1000000;
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  if (local_d15 == '\0') {
LAB_00495f90:
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)local_d00);
    EncodeOutgoingPacketField((void *)(param_1 + 0x10),uVar5);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    local_cf8 = param_1 + 0x1072;
  }
  else {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    local_cf8 = param_1 + 0x1072;
    iVar4 = PeekPacketChecksumState((void *)local_cf8);
    local_d15 = iVar4 == -1000000;
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    if (local_d15 == '\0') goto LAB_00495f90;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x10));
    EncodeOutgoingPacketField((void *)local_d00,uVar5);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)local_d00);
    EncodeOutgoingPacketField((void *)(param_1 + 0x10fb),uVar5);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x99));
    EncodeOutgoingPacketField((void *)local_cf8,uVar5);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  }
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x1072));
  EncodeOutgoingPacketField((void *)(param_1 + 0x99),uVar5);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d14 = PeekPacketChecksumState((void *)local_d00);
  iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x122));
  EncodeOutgoingPacketField((void *)local_d00,iVar4 + local_d14);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d14 = PeekPacketChecksumState((void *)local_cf8);
  iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x1ab));
  EncodeOutgoingPacketField((void *)local_cf8,iVar4 + local_d14);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d14 = PeekPacketChecksumState((void *)(param_1 + 0x10));
  iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x122));
  EncodeOutgoingPacketField((void *)(param_1 + 0x10),iVar4 + local_d14);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d14 = PeekPacketChecksumState((void *)(param_1 + 0x99));
  iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x1ab));
  EncodeOutgoingPacketField((void *)(param_1 + 0x99),iVar4 + local_d14);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  FUN_004585e0();
  EncodeChecksumDeltaAdd();
  local_4 = 0;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d14 = PeekPacketChecksumState((void *)&DAT_00e9bed8);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = EncodeChecksumDeltaMod();
  SUBFIELD(local_4,0,undefined1) = 1;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d04 = PeekPacketChecksumState((void *)uVar5);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = (uint)SUBFIELD(local_4,1,undefined3) << 8;
  if (local_cd0 != 0) {
    ScrambleChecksumGuardBytes(local_cd0,&g_valueGuardKeyTable);
    local_d14 = local_cd0;
    TreeLowerBound();
    pcVar18 = (code *)LeaveCriticalSection;
  }
  local_4 = 0xffffffff;
  if (local_aac != 0) {
    ScrambleChecksumGuardBytes(local_aac,&g_valueGuardKeyTable);
    local_d14 = local_aac;
    TreeLowerBound();
    pcVar18 = (code *)LeaveCriticalSection;
  }
  local_d14 = param_1[0xfe7] % 0x168;
  iVar4 = FloatToInt64();
  local_d14 = FloatToInt64();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x10));
  EncodeOutgoingPacketField((void *)(param_1 + 0x10),(iVar4 << 8) + uVar5);
  puVar33 = &g_valueGuardLock;
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x99));
  EncodeOutgoingPacketField((void *)(param_1 + 0x99),(local_d14 << 8) + uVar5);
  puVar32 = &g_valueGuardLock;
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d04 = PeekPacketChecksumState((void *)(param_1 + 0x122));
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_d10[0] = PeekPacketChecksumState((void *)(param_1 + 0x1ab));
  puVar31 = &g_valueGuardLock;
  (*pcVar18)();
  uVar5 = EncodeChecksumPairDiff(param_1 + 0x10,auStack_cf4,param_1 + 0x10fb);
  uStack_14 = 2;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)uVar5);
  EncodeOutgoingPacketField((void *)(param_1 + 0x122),uVar6);
  piVar29 = (int *)&g_valueGuardLock;
  (*pcVar18)(&g_valueGuardLock);
  uStack_18 = 0xffffffff;
  if (local_ce4 != 0) {
    ScrambleChecksumGuardBytes(local_ce4,&g_valueGuardKeyTable);
    TreeLowerBound(&stack0xfffff2e0,&g_valueGuardMap);
    pcVar18 = (code *)LeaveCriticalSection;
  }
  uVar5 = EncodeChecksumPairDiff(param_1 + 0x99,&local_cf8,param_1 + 0x1184);
  uStack_18 = 3;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)uVar5);
  EncodeOutgoingPacketField((void *)(param_1 + 0x1ab),uVar6);
  (*pcVar18)(&g_valueGuardLock);
  uStack_1c = 0xffffffff;
  if (puStack_ce8 != (undefined *)0x0) {
    ScrambleChecksumGuardBytes();
    TreeLowerBound(&stack0xfffff2dc,&g_valueGuardMap);
    pcVar18 = (code *)LeaveCriticalSection;
    puVar33 = puStack_ce8;
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x10));
  EncodeOutgoingPacketField((void *)(param_1 + 0x10fb),uVar6);
  puVar28 = &g_valueGuardLock;
  (*pcVar18)(&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x99));
  EncodeOutgoingPacketField((void *)(param_1 + 0x1184),uVar6);
  puVar26 = (undefined4 *)&g_valueGuardLock;
  (*pcVar18)(&g_valueGuardLock);
  FUN_004585e0();
  (**(code **)(*param_1 + 0x14))();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EncodeOutgoingPacketField((void *)(param_1 + 0x122),local_d04);
  cVar3 = '\0';
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EncodeOutgoingPacketField((void *)(param_1 + 0x1ab),local_d10[0]);
  puVar25 = &g_valueGuardLock;
  (*pcVar18)(&g_valueGuardLock);
  cVar2 = PeekPacketChecksumBool();
  if (cVar2 == '\0') {
    uVar6 = EncodeChecksumDeltaShr(param_1 + 0x10,local_d10,8);
    uStack_30 = 9;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)uVar6);
    EncodeOutgoingPacketField((void *)(param_1 + 0x3d5),uVar5);
    (*pcVar18)(&g_valueGuardLock);
    piVar30 = local_d00;
    uStack_34 = 0xffffffff;
    if (local_d00 != (int *)0x0) {
      ScrambleChecksumGuardBytes();
      TreeLowerBound(&stack0xfffff2dc,&g_valueGuardMap);
      pcVar18 = (code *)LeaveCriticalSection;
      piVar29 = piVar30;
    }
    uVar6 = EncodeChecksumDeltaShr(param_1 + 0x99,&local_d14,8);
    uStack_34 = 10;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)uVar6);
    EncodeOutgoingPacketField((void *)(param_1 + 0x45e),uVar5);
    cVar2 = '\0';
    (*pcVar18)();
    uStack_38 = 0xffffffff;
    if (local_d04 != 0) {
      ScrambleChecksumGuardBytes(local_d04,&g_valueGuardKeyTable);
      TreeLowerBound(&stack0xfffff2d8,&g_valueGuardMap);
      pcVar18 = (code *)LeaveCriticalSection;
    }
    cVar3 = CheckGuardedBoolAnd(param_1[0x3d2] != 0,(byte *)(param_1 + 0x3d3));
    if (cVar3 == '\0') {
      EmitChecksumSum(param_1 + 0x122, param_1 + 0x2bd);
      EmitChecksumSum(param_1 + 0x1ab, param_1 + 0x346);
    }
    else {
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar14 = PeekPacketChecksumState((void *)(param_1 + 0x122));
      (*pcVar18)(&g_valueGuardLock);
      if ((int)((uVar14 ^ (int)uVar14 >> 0x1f) - ((int)uVar14 >> 0x1f)) < 200) {
        cVar3 = PacketChecksumLessThan(param_1 + 0x122,0);
        if (cVar3 == '\0') {
          uVar5 = 200;
        }
        else {
          uVar5 = 0xffffff38;
        }
        QueueOutgoingPacketField(uVar5);
      }
      uVar5 = EncodeChecksumDeltaDiv(param_1[0x3d2] + 0x25c,auStack_8d0,4);
      uStack_38 = 0xb;
      uVar5 = EncodeChecksumPairDiff(param_1[0x3d2] + 0x38,auStack_488,uVar5);
      uStack_38 = 0xc;
      puVar28 = (undefined *)0x3;
      cVar3 = CompareChecksumPair(param_1 + 0x3d5,uVar5);
      if (cVar3 == '\0') {
        uVar5 = EncodeChecksumDeltaDiv(param_1[0x3d2] + 0x25c,auStack_af4,4);
        uStack_38 = 0xd;
        uVar5 = EncodeChecksumPairSum(param_1[0x3d2] + 0x38,auStack_d18,uVar5);
        uStack_38 = 0xe;
        puVar28 = (undefined *)0xf;
        cVar3 = CompareChecksumPair(uVar5,param_1 + 0x3d5);
        bVar20 = false;
        if (cVar3 != '\0') goto LAB_004972ef;
      }
      else {
LAB_004972ef:
        bVar20 = true;
      }
      uStack_38 = 0xd;
      if (((uint)puVar28 & 8) != 0) {
        puVar28 = (undefined *)((uint)puVar28 & 0xfffffff7);
        ScrubChecksumGuard();
      }
      uStack_38 = 0xc;
      if (((uint)puVar28 & 4) != 0) {
        puVar28 = (undefined *)((uint)puVar28 & 0xfffffffb);
        ScrubChecksumGuard();
      }
      uStack_38 = 0xb;
      if (((uint)puVar28 & 2) != 0) {
        puVar28 = (undefined *)((uint)puVar28 & 0xfffffffd);
        ScrubChecksumGuard();
      }
      uStack_38 = 0xffffffff;
      if (((uint)puVar28 & 1) != 0) {
        ScrubChecksumGuard();
      }
      if (bVar20) {
        iVar4 = param_1[0x3d4];
        param_1[0x3d4] = iVar4 + 1;
        if (iVar4 + 1 < 3) {
          uVar5 = EncodeChecksumNegate(param_1 + 0x122,auStack_8d0);
          uStack_38 = 0xf;
          EncodeChecksumState(uVar5);
          uStack_38 = 0xffffffff;
          ScrubChecksumGuard();
        }
        else {
          SetGuardedBool(0,GB_GUARD_UNRECOVERED);
        }
      }
    }
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    (*pcVar18)(&g_valueGuardLock);
    if (cVar2 == '\0') {
      iVar4 = *(int *)(&g_nCameraBoundX + g_clientContext);
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar13 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
      cVar2 = iVar4 <= iVar13;
      (*pcVar18)(&g_valueGuardLock);
      if (cVar2 != '\0') goto LAB_004974a8;
      cVar2 = PacketChecksumGreaterEqual(param_1 + 0x45e,*(undefined4 *)(&g_nCameraBoundY + g_clientContext));
      if ((cVar2 != '\0') || (cVar2 = PacketChecksumLessThan(param_1 + 0x45e,0xfffffc18), cVar2 != '\0'))
      goto LAB_004974a8;
    }
    else {
LAB_004974a8:
      *(undefined1 *)(param_1 + 5) = 1;
      cVar2 = PeekPacketChecksumBool((byte *)param_1 + 0xf3f);
      if ((cVar2 != '\0') && (cVar2 = InitChecksumSeed(), cVar2 == '\0')) {
        iVar4 = *(int *)(*(int *)(g_clientContext + 0x621e4) + 0x24);
        if ((iVar4 == 5) || (uVar5 = 2, iVar4 == 6)) {
          uVar5 = 0x19;
        }
        QueueOutgoingPacketField(uVar5);
      }
    }
    cVar2 = CheckBothGuardedBools();
    if (cVar2 != '\0') {
      uVar6 = EncodeChecksumDeltaShr(piVar29,auStack_274,8);
      uStack_48 = 0x10;
      uVar5 = EncodeChecksumDeltaShr(puVar28,auStack_6bc,8);
      SUBFIELD(uStack_48,0,undefined1) = 0x11;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar13 = PeekPacketChecksumState((void *)uVar6);
      (*pcVar18)(&g_valueGuardLock);
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar16 = PeekPacketChecksumState((void *)uVar5);
      (*pcVar18)(&g_valueGuardLock);
      iVar4 = g_clientContext;
      piVar29 = (int *)(&DAT_006a7708 + g_clientContext);
      cVar2 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
      if ((cVar2 == '\0') && ((&DAT_006a7758)[iVar4] != '\0')) {
        if (((&DAT_006a7736)[iVar4] == '\x01') &&
           ((uVar14 = iVar13 - *(int *)(&g_nCameraY + iVar4) >> 0x1f,
            200 < (int)((iVar13 - *(int *)(&g_nCameraY + iVar4) ^ uVar14) - uVar14) ||
            (uVar14 = iVar16 - *(int *)(&g_nCameraX + iVar4) >> 0x1f,
            300 < (int)((iVar16 - *(int *)(&g_nCameraX + iVar4) ^ uVar14) - uVar14))))) {
          (&DAT_006a7736)[iVar4] = 0;
        }
        iVar8 = 400;
        if (399 < iVar16) {
          iVar8 = iVar16;
        }
        iVar9 = *(int *)(&g_nCameraBoundX + iVar4) + -400;
        if ((iVar8 <= iVar9) && (iVar9 = iVar16, iVar16 < 400)) {
          iVar9 = 400;
        }
        iVar16 = -0x14;
        if (-0x15 < iVar13) {
          iVar16 = iVar13;
        }
        iVar8 = *(int *)(&g_nCameraBoundY + iVar4) + -0x104;
        if ((iVar16 <= *(int *)(&g_nCameraBoundY + iVar4) + -0x104) && (iVar8 = -0x14, -0x15 < iVar13))
        {
          iVar8 = iVar13;
        }
        iVar13 = *piVar29 - iVar9;
        *(int *)(&g_nCameraScrollY + iVar4) = iVar8;
        if (((*(int *)(&DAT_006a770c + iVar4) - iVar8) * (*(int *)(&DAT_006a770c + iVar4) - iVar8) +
             iVar13 * iVar13 < 40000) &&
           (((&DAT_006a7734)[iVar4] == '\x01' || ((&DAT_006a7f6c)[iVar4] == '\0')))) {
          *(int *)(&g_nCameraX + iVar4) = iVar9;
          *piVar29 = iVar9;
          *(int *)(&g_nCameraY + iVar4) = iVar8;
          *(int *)(&DAT_006a770c + iVar4) = iVar8;
        }
        if ((((&DAT_006a7734)[iVar4] == '\0') && ((&DAT_006a7f6c)[iVar4] == '\x01')) &&
           ((&DAT_006a7736)[iVar4] == '\0')) {
          iVar13 = *(int *)(&DAT_006a7730 + iVar4);
          iVar16 = iVar13;
          if (iVar9 <= iVar13) {
            iVar16 = iVar9;
          }
          iVar15 = *(int *)(&DAT_006a772c + iVar4);
          if ((iVar15 <= iVar16) && (iVar15 = iVar13, iVar9 <= iVar13)) {
            iVar15 = iVar9;
          }
          *(int *)(&g_nCameraScrollX + iVar4) = iVar15;
        }
        if ((&DAT_006a7734)[iVar4] == '\x01') {
          *(int *)(&g_nCameraScrollX + iVar4) = iVar9;
          *(int *)(&g_nCameraScrollY + iVar4) = iVar8;
        }
      }
      uStack_48 = CONCAT31(SUBFIELD(uStack_48,1,undefined3),0x10);
      if (iStack_6a8 != 0) {
        ScrambleChecksumGuardBytes(iStack_6a8,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff2c8,&g_valueGuardMap);
      }
      uStack_48 = 0xffffffff;
      if (iStack_260 != 0) {
        ScrambleChecksumGuardBytes(iStack_260,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff2c8,&g_valueGuardMap);
      }
    }
    goto LAB_00497710;
  }
  SyncOutgoingChecksumField(param_1 + 0x3d5, param_1[2],param_1 + 0x45e);
  cVar2 = PeekPacketChecksumBool((byte *)param_1 + 0x3918);
  if (cVar2 != '\0') {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    puVar28 = &g_valueGuardLock;
    (*pcVar18)(&g_valueGuardLock);
    if ((-1 < iVar4) && (iVar4 < *(int *)(&g_nCameraBoundX + g_clientContext))) {
      pcVar7 = (char *)(*(int *)(&DAT_006a773c + g_clientContext) + iVar4);
      iVar4 = 0;
      if (0 < *(int *)(&g_nCameraBoundY + g_clientContext)) {
        do {
          if (*pcVar7 != '\0') goto LAB_004964ea;
          pcVar7 = pcVar7 + *(int *)(&g_nCameraBoundX + g_clientContext);
          iVar4 = iVar4 + 1;
        } while (iVar4 < *(int *)(&g_nCameraBoundY + g_clientContext));
      }
    }
    iVar4 = 10000;
LAB_004964ea:
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    (*pcVar18)(&g_valueGuardLock);
    FUN_00436070(&DAT_006a7f70 + g_clientContext,puVar26,uVar6);
    puVar31 = (undefined *)0x1;
    if (0 < param_1[0xfe5]) {
      puVar31 = (undefined *)param_1[0xfe5];
    }
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    puVar26 = (undefined4 *)PeekPacketChecksumState((void *)&DAT_00e55ab8);
    (*pcVar18)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)&DAT_00e55ab8);
    (*pcVar18)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)&DAT_007949c8);
    (*pcVar18)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    (*pcVar18)(&g_valueGuardLock);
    ApplyBlastDamage(puVar33,puVar25,0,puVar32,iVar4 / (int)puVar26,puVar28,(char)param_1[0xf]);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uImpactPower = PeekPacketChecksumState((void *)(param_1 + 0x5f9));
    (*pcVar18)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iImpactX = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    (*pcVar18)(&g_valueGuardLock);
    uVar23 = 0;
    uVar22 = 2;
    uVar21 = 0xff;
    uVar6 = 0;
    PeekPacketChecksumBool((byte *)param_1 + 0xf3c);
    SpawnBlastEffect(iVar4,iImpactX,(char)param_1[0xf],7,0,uVar6,uImpactPower,uVar21,uVar22,uVar23);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  cVar24 = '\0';
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  cVar2 = '\0';
  (*pcVar18)();
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(g_clientContext + 0xebcbc));
  (*pcVar18)(&g_valueGuardLock);
  if (cVar3 == '\0') {
    puVar26 = operator_new(0x3fbc);
    uStack_3c = 4;
    if (puVar26 == (undefined4 *)0x0) {
      puVar26 = (undefined4 *)0x0;
    }
    else {
      InitProjectile(puVar26,0x186a9);
      *puVar26 = &PTR_FUN_0055658c;
      puVar26[0xfe7] = 0xffffffff;
    }
    uStack_3c = 0xffffffff;
    iVar13 = *(int *)(*(int *)(&DAT_006a7f8c + g_clientContext) + 0x1c);
    uVar14 = *(uint *)(iVar13 + 4);
    while (uVar14 < 0x186ab) {
      if (uVar14 == 0x186aa) {
        piVar29 = *(int **)(iVar13 + 0x10);
        bVar20 = true;
        if (piVar29[2] == 0) goto LAB_0049676b;
        break;
      }
      iVar13 = *(int *)(iVar13 + 0x1c);
      uVar14 = *(uint *)(iVar13 + 4);
    }
  }
  goto LAB_00496abf;
  while( true ) {
    piVar29 = (int *)piVar29[4];
    bVar20 = piVar29[2] == 0;
    if (!bVar20) break;
LAB_0049676b:
    if (bVar20) {
      iVar13 = piVar29[0xf];
      iVar16 = piVar29[0xe];
      iVar8 = CalculateAngleFromDelta();
      iVar9 = FloatToInt64();
      iVar16 = iVar16 - iVar9;
      iVar9 = FloatToInt64();
      (**(code **)(*piVar29 + 4))(&DAT_00553b90);
      piVar29[0x11] = iVar8;
      cVar3 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
      if (cVar3 == '\0') {
        uStack_8d8 = 0;
        uStack_ae4 = 0;
        EncodeOutgoingPacketField((void *)auStack_af8,0);
        uStack_3c = 7;
        uStack_afc = 0;
        uStack_d08 = 0;
        EncodeOutgoingPacketField((void *)&stack0xfffff2e4,0);
        SUBFIELD(uStack_3c,0,undefined1) = 8;
        SyncOutgoingChecksumField(auStack_af8, iVar4 + 0x10,&stack0xfffff2e4);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        PeekPacketChecksumState((void *)&stack0xfffff2e4);
        (*pcVar18)(&g_valueGuardLock);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        puVar26 = (undefined4 *)PeekPacketChecksumState((void *)auStack_af8);
        (*pcVar18)(&g_valueGuardLock);
        iVar4 = param_1[0xfe5];
        uVar10 = 0;
        uVar23 = 0xff;
        uVar22 = 100;
        uVar21 = 0;
        PeekPacketChecksumBool((byte *)param_1 + 0xf3c);
        FUN_00432320((char)param_1[0xf],1,1,uVar6,uVar21,uVar22,uVar23,iVar4,uVar10);
        uStack_3c = CONCAT31(SUBFIELD(uStack_3c,1,undefined3),7);
        ScrubChecksumGuard();
LAB_00496a28:
        uStack_3c = 0xffffffff;
        ScrubChecksumGuard();
        puVar27 = puVar26;
      }
      else {
        cVar3 = FUN_004e4fe0(iVar16,&DAT_006a7708 + g_clientContext,&stack0xfffff2e0,&stack0xfffff2dc,2,1,1,iVar13 - iVar9);
        puVar27 = puVar26;
        if (cVar3 != '\0') {
          uStack_afc = 0;
          uStack_d08 = 0;
          EncodeOutgoingPacketField((void *)&stack0xfffff2e4,0);
          uStack_3c = 5;
          uStack_8d8 = 0;
          uStack_ae4 = 0;
          EncodeOutgoingPacketField((void *)auStack_af8,0);
          SUBFIELD(uStack_3c,0,undefined1) = 6;
          QueueOutgoingPacketField(puVar31);
          QueueOutgoingPacketField(uVar5);
          SyncOutgoingChecksumField(&stack0xfffff2e4, iVar4 + 0x10,auStack_af8);
          iVar4 = param_1[0xfe5];
          uVar10 = 0;
          uVar23 = 0xff;
          uVar22 = 100;
          uVar21 = 0;
          PeekChecksumStateUnderLock(auStack_af8);
          uVar6 = PeekChecksumStateUnderLock(&stack0xfffff2e4);
          PeekPacketChecksumBool((byte *)param_1 + 0xf3c);
          FUN_00432320((char)param_1[0xf],1,1,uVar6,uVar21,uVar22,uVar23,iVar4,uVar10);
          uStack_3c = CONCAT31(SUBFIELD(uStack_3c,1,undefined3),5);
          ScrubChecksumGuard();
          goto LAB_00496a28;
        }
      }
      puVar27[0xfea] = iVar16;
      puVar27[0xfeb] = iVar13 - iVar9;
      puVar27[0xfec] = puVar31;
      puVar27[0xfed] = uVar5;
      puVar27[0xfee] = iVar8;
      puVar26 = puVar27;
      /* FIXED (2026-07-15): dropped self arg - angr-confirmed EDI==piVar29
       * (the traversal pointer set by piVar29=piVar29[4] above and never
       * reassigned before this point) at the real 0x496a74/0x496a81 calls. */
      uVar5 = FUN_004ac4d0(piVar29);
      puVar27[0xfe9] = uVar5;
      iVar4 = FUN_004ac400(piVar29);
      puVar27[0xfe8] = iVar4 * param_1[0xfe5];
      /* guard-cell: proven - the ctx+0x6a7f70 spill/fold chain; see the
       * fold-dominance proof in commit dc092b4^..'s successor (lea
       * REG,[ctx+0x6a7f70] -> slot, +4 fold -> slot, this peek reads it
       * with no interposed writer and no bypass edge). */
      cVar3 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
      if (cVar3 == '\0') {
        RegisterActiveObject(0, 0, (undefined4 *)0);
      }
      else {
        (**(code **)*puVar27)(1);
      }
      break;
    }
  }
LAB_00496abf:
  *(undefined1 *)(param_1 + 5) = 1;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x1ab));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x122));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar21 = PeekPacketChecksumState((void *)(param_1 + 0xbdd));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar22 = PeekPacketChecksumState((void *)(param_1 + 0xb54));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar23 = PeekPacketChecksumState((void *)(param_1 + 0xacb));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar10 = PeekPacketChecksumState((void *)(param_1 + 0xa42));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar11 = PeekPacketChecksumState((void *)(param_1 + 0x9b9));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar12 = PeekPacketChecksumState((void *)(param_1 + 0x930));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iCraterY = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iCraterX = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  ApplyCraterExcavation(uVar12,uVar11,uVar10,uVar23,uVar22,uVar21,uVar6,uVar5,
                 (int)(&DAT_006a7708 + g_clientContext),(int)iCraterX,(int)iCraterY);
  RebuildTerrainColumnCache(&DAT_006a7708 + g_clientContext);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  FUN_00450eb0(uVar6,uVar5,param_1,1,0,0);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  SpawnCrashEffect(uVar6, uVar5, param_1[0xfe4]);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  SpawnFlameEffect(param_1[2],uVar6,uVar5,param_1[0xe25],param_1 + 0xe26);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x570));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar21 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  FUN_00437150(uVar21,uVar6,0x3c,uVar5);
  iVar4 = g_clientContext;
  cVar3 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
  if (cVar3 != '\x01') {
    iVar13 = *(int *)(&DAT_006a7750 + iVar4);
    if (iVar13 < 0xb) {
      iVar13 = 10;
    }
    *(int *)(&DAT_006a7750 + iVar4) = iVar13;
    iVar13 = *(int *)(&DAT_006a7754 + iVar4);
    if (iVar13 < 0xb) {
      iVar13 = 10;
    }
    *(int *)(&DAT_006a7754 + iVar4) = iVar13;
  }
  cVar3 = PeekPacketChecksumBool();
  if (cVar3 == '\0') {
    AcquireSoundChannel(0);
  }
  cVar3 = PeekPacketChecksumBool((byte *)param_1 + 0xf3f);
  pcVar18 = (code *)LeaveCriticalSection;
  if ((cVar3 != '\0') &&
     (cVar3 = InitChecksumSeed(), pcVar18 = (code *)LeaveCriticalSection, cVar3 == '\0')) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)&DAT_007949c8);
    pcVar18 = (code *)LeaveCriticalSection;
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    QueueOutgoingPacketField(uVar5);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(g_clientContext + 0x5b85c));
  PeekPacketChecksumState((void *)(param_1 + 0xe48));
  (*pcVar18)(&g_valueGuardLock);
  cVar3 = CheckGuardedBoolAnd(puVar26);
  if (cVar3 != '\0') {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)(param_1 + 0xe48));
    EncodeOutgoingPacketField((void *)(g_clientContext + 0x5b85c),uVar5);
    (*pcVar18)(&g_valueGuardLock);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(param_1 + 0x234));
  (*pcVar18)(&g_valueGuardLock);
  if (cVar24 == '\0') {
LAB_00496f3d:
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x234));
    cVar3 = 0 < iVar4;
    (*pcVar18)(&g_valueGuardLock);
    if (cVar3 != '\0') {
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x122));
      cVar3 = iVar4 < 0;
      (*pcVar18)(&g_valueGuardLock);
      if (cVar3 != '\0') goto LAB_00496f89;
    }
  }
  else {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x122));
    cVar3 = 0 < iVar4;
    (*pcVar18)(&g_valueGuardLock);
    if (cVar3 == '\0') goto LAB_00496f3d;
LAB_00496f89:
    cVar3 = PeekPacketChecksumBool((byte *)param_1 + 0xf4c);
    if (cVar3 == '\0') {
      SetGuardedBool(1,GB_GUARD_UNRECOVERED);
    }
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  PeekPacketChecksumState((void *)(param_1 + 0xed2));
  (*pcVar18)(&g_valueGuardLock);
  if ((cVar2 == '\0') || (cVar2 = CompareChecksumPair(param_1 + 0xed2,param_1 + 0xf5b), cVar2 == '\0')) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar4 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    iVar13 = PeekPacketChecksumState((void *)(param_1 + 0xed2));
    cVar2 = iVar13 < iVar4;
    (*pcVar18)(&g_valueGuardLock);
    if ((cVar2 == '\0') ||
       ((cVar2 = CompareChecksumExceeds(param_1 + 0xed2,param_1 + 0xf5b), cVar2 == '\0' ||
        (param_1[0xed1] < 6)))) goto LAB_00497710;
  }
  iVar4 = g_clientContext;
  pbVar19 = (byte *)(g_clientContext + 0x62143);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar13 = _rand();
  *pbVar19 = (byte)iVar13;
  iVar13 = _rand();
  bVar1 = *pbVar19;
  *(byte *)(iVar4 + 0x62144) = (byte)iVar13;
  bVar17 = '\x01' << (bVar1 & 7);
  bVar1 = *pbVar19;
  bVar17 = ~bVar17 & (byte)iVar13 | bVar17;
  *(byte *)(iVar4 + 0x62144) = bVar17;
  *(byte *)(iVar4 + 0x62145) = bVar17 + bVar1 + -0x34;
  (*pcVar18)(&g_valueGuardLock);
LAB_00497710:
  *unaff_FS_OFFSET = uStack_50;
  return;
}

