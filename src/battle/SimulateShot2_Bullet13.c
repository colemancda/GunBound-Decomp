/* SimulateShot2_Bullet13 - 0x0049e650 in the original binary.
 *
 * RENAMED (2026-08-16, from FUN_0049e650): CProjectile virtual slot 5
 * (SimulateProjectileFrame, base impl 0x455cc0) override for mobile type
 * 12's shot 2 (weapon index 1, texture bullet13s); vtable 0x55649c, ctor
 * InitShot2_Bullet13 reached only through SpawnPrimaryShot's case 12. Type/role
 * CONFIRMED via vtable geometry (tools/projectile_class_map.py); the
 * mobile NAME is not confirmable (no mobile-type->name map in the repo).
 *
 * Body is a raw/near-verbatim Ghidra port, not hand-verified. See
 * src/README.md's "Raw/verbatim ports" section for status.
 *
 * DROPPED-CELL FIX (2026-08-16, CValueGuard sweep): recovered the guard
 * cell at all 20 argless PeekPacketChecksumState() calls and all 22 1-arg
 * EncodeOutgoingPacketField() calls (42 sites).  Same shape as
 * SimulateSuperShot_Bullet13 / SimulateShot2_Bullet11 /
 * SimulatePrimaryShot_Bullet14, with one extra four-Encode group: the
 * delta/pair helpers return their arg2 scratch cell, so each Peek that
 * follows one reads that C local (local_1574 / local_ac0 / local_112c /
 * local_89c / local_454 / local_1350 / local_ce4 / local_f08 /
 * local_230 / local_678).  The block-head peek pairs and the lone Encodes
 * use frame[0x48] = the C's local_158c = local_159c + 0x15c0 =
 * param_1 + 0x15c0; the four-Encode groups write the position cells
 * param_1 + 0x10/0x99/0x3d5/0x45e (this = frame[0x38]/EBX).
 */
#include "ghidra_types.h"


/* WARNING: Function: __chkstk replaced with injection: alloca_probe */

void __thiscall SimulateShot2_Bullet13(int param_1,int param_2)

{
  int *piVar1;
  char cVar2;
  undefined4 uVar3;
  int iVar4;
  int iVar5;
  byte bVar6;
  byte bVar7;
  uint uVar8;
  int iVar9;
  int iVar10;
  byte *pbVar11;
  undefined4 *unaff_FS_OFFSET;
  bool bVar12;
  int local_15c4;
  int local_15c0;
  uint local_15bc;
  int local_15b8;
  int local_15b4;
  char local_15ad;
  int local_15ac;
  int local_15a8 [2];
  char local_159d;
  int local_159c;
  int local_1598;
  int local_1594;
  int local_1590;
  int local_158c;
  int local_1588;
  int local_1584;
  undefined1 local_1580 [8];
  int local_1578;
  undefined1 local_1574 [548];
  undefined1 local_1350 [548];
  undefined1 local_112c [548];
  undefined1 local_f08 [548];
  undefined1 local_ce4 [548];
  undefined1 local_ac0 [548];
  undefined1 local_89c [548];
  undefined1 local_678 [548];
  undefined1 local_454 [548];
  undefined1 local_230 [548];
  undefined4 uStack_10;
  undefined4 local_c;
  undefined1 *puStack_8;
  undefined4 local_4;
  
  local_4 = 0xffffffff;
  puStack_8 = &LAB_0053ee50;
  local_c = *unaff_FS_OFFSET;
  *unaff_FS_OFFSET = &local_c;
  uStack_10 = 0x49e66f;
  local_1584 = param_1 + 0x40;
  local_15bc = 0;
  local_15ad = '\0';
  local_159c = param_1;
  EncodeChecksumDeltaShr(local_1584,local_1574,8);
  local_4 = 0;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_1598 = PeekPacketChecksumState((void *)(local_1574));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  local_15b4 = (*(int *)(local_1574 + 0x14));
  if ((*(int *)(local_1574 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_1574 + 0x14),&g_valueGuardKeyTable);
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  local_1594 = param_1 + 0x264;
  EncodeChecksumDeltaShr(local_1594,local_1574,8);
  local_4 = 1;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_158c = PeekPacketChecksumState((void *)(local_1574));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 0xffffffff;
  local_15b4 = (*(int *)(local_1574 + 0x14));
  if ((*(int *)(local_1574 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_1574 + 0x14),&g_valueGuardKeyTable);
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  uVar3 = EncodeChecksumPairDiff(local_1584,local_ac0,param_1 + 0x488);
  local_4 = 2;
  EncodeChecksumDeltaShr(uVar3,local_1574,8);
  local_4 = 3;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_1590 = PeekPacketChecksumState((void *)(local_1574));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar4 = (*(int *)(local_1574 + 0x14));
  local_4 = 2;
  if ((*(int *)(local_1574 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_1574 + 0x14),&g_valueGuardKeyTable);
    local_15b4 = iVar4;
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  iVar4 = (*(int *)(local_ac0 + 0x14));
  local_4 = 0xffffffff;
  if ((*(int *)(local_ac0 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_ac0 + 0x14),&g_valueGuardKeyTable);
    local_15b4 = iVar4;
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  uVar3 = EncodeChecksumPairDiff(local_1594,local_1574,local_159c + 0x6ac);
  local_4 = 4;
  EncodeChecksumDeltaShr(uVar3,local_ac0,8);
  local_4 = 5;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar4 = PeekPacketChecksumState((void *)(local_ac0));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  local_4 = 4;
  local_15b4 = (*(int *)(local_ac0 + 0x14));
  if ((*(int *)(local_ac0 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_ac0 + 0x14),&g_valueGuardKeyTable);
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  local_4 = 0xffffffff;
  local_15b4 = (*(int *)(local_1574 + 0x14));
  if ((*(int *)(local_1574 + 0x14)) != 0) {
    ScrambleChecksumGuardBytes(*(int *)(local_1574 + 0x14),&g_valueGuardKeyTable);
    TreeLowerBound(local_15a8,&g_valueGuardMap);
  }
  uVar8 = local_1590 - local_1598 >> 0x1f;
  iVar5 = (local_1590 - local_1598 ^ uVar8) - uVar8;
  uVar8 = iVar4 - local_158c >> 0x1f;
  iVar9 = (iVar4 - local_158c ^ uVar8) - uVar8;
  local_1584 = iVar5 + 1;
  iVar10 = iVar9 + 1;
  if (iVar10 < local_1584) {
    local_1584 = ((local_158c - iVar4) * 0x10000) / local_1584;
    local_1588 = iVar4 << 0x10;
    if (local_1598 < local_1590) {
      local_15ac = local_1590 + -1;
      local_1590 = -1;
    }
    else {
      local_15ac = local_1590 + 1;
      local_1590 = 1;
    }
    local_15a8[0] = iVar5 + param_2;
    local_1594 = 0;
    if (0 < local_15a8[0]) {
      local_15b4 = local_1584 * param_2;
      local_158c = local_159c + 0x15c0;
      local_1578 = local_1590 * param_2;
      local_1598 = local_15ac;
      do {
        local_1588 = local_1588 + local_15b4;
        local_15c0 = local_1588 >> 0x10;
        local_15c4 = local_15ac;
        if ((local_15ad == '\0') &&
           (iVar5 = FUN_004510f0(local_15ac,(int)(&DAT_006a7f88 + g_clientContext)), iVar4 = local_158c, iVar5 != 0)) {
          local_15b8 = EncodeChecksumDeltaAdd(local_158c,local_112c,param_2);
          local_4 = 6;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_15b8 = PeekPacketChecksumState((void *)(local_112c));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (local_15b8 < 0x14) {
LAB_0049ea8b:
            local_15b8 = EncodeChecksumDeltaAdd(iVar4,local_89c,param_2);
            local_4 = 8;
            local_15bc = local_15bc | 2;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            local_15b8 = PeekPacketChecksumState((void *)(local_89c));
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            if (local_15b8 < 0x14) {
              uVar3 = 0x14;
            }
            else {
              EncodeChecksumDeltaAdd(iVar4,local_454,param_2);
              local_4 = 9;
              local_15bc = local_15bc | 4;
              EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
              uVar3 = PeekPacketChecksumState((void *)(local_454));
              LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            }
          }
          else {
            local_15b8 = EncodeChecksumDeltaAdd(iVar4,local_1350,param_2);
            local_4 = 7;
            local_15bc = local_15bc | 1;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            local_15b8 = PeekPacketChecksumState((void *)(local_1350));
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            if (local_15b8 < 0x65) goto LAB_0049ea8b;
            uVar3 = 100;
          }
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(local_158c), uVar3);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar4 = (*(int *)(local_454 + 0x14));
          local_4 = 8;
          if (((local_15bc & 4) != 0) && (local_15bc = local_15bc & 0xfffffffb, (*(int *)(local_454 + 0x14)) != 0)) {
            ScrambleChecksumGuardBytes();
            local_15b8 = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_89c + 0x14));
          local_4 = 7;
          if (((local_15bc & 2) != 0) && (local_15bc = local_15bc & 0xfffffffd, (*(int *)(local_89c + 0x14)) != 0)) {
            ScrambleChecksumGuardBytes();
            local_15b8 = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_1350 + 0x14));
          local_4 = 6;
          if (((local_15bc & 1) != 0) && (local_15bc = local_15bc & 0xfffffffe, (*(int *)(local_1350 + 0x14)) != 0)) {
            ScrambleChecksumGuardBytes();
            local_15b8 = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_112c + 0x14));
          local_4 = 0xffffffff;
          if ((*(int *)(local_112c + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_112c + 0x14),&g_valueGuardKeyTable);
            local_15b8 = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          local_15ad = '\x01';
        }
        iVar4 = local_159c;
        pbVar11 = (byte *)(local_159c + 0xf4c);
        cVar2 = DecodeGuardedBool(pbVar11);
        if (cVar2 != '\0') {
          iVar5 = FUN_00451030(local_15ac,(int)(&DAT_006a7f88 + g_clientContext));
          *(int *)(iVar4 + 0xf48) = iVar5;
          if (iVar5 != 0) {
            *(undefined4 *)(iVar4 + 0xf50) = 0;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar5 = _rand();
            *pbVar11 = (byte)iVar5;
            iVar5 = _rand();
            bVar7 = *pbVar11;
            *(byte *)(iVar4 + 0xf4d) = (byte)iVar5;
            bVar6 = '\x01' << (bVar7 & 7);
            bVar7 = *pbVar11;
            bVar6 = ~bVar6 & (byte)iVar5 | bVar6;
            *(byte *)(iVar4 + 0xf4d) = bVar6;
            *(byte *)(iVar4 + 0xf4e) = bVar6 + bVar7 + -0x34;
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar5 = g_clientContext;
            pbVar11 = (byte *)(g_clientContext + 0x62140);
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar9 = _rand();
            *pbVar11 = (byte)iVar9;
            iVar9 = _rand();
            bVar7 = *pbVar11;
            *(byte *)(iVar5 + 0x62141) = (byte)iVar9;
            bVar6 = '\x01' << (bVar7 & 7);
            bVar7 = *pbVar11;
            bVar6 = ~bVar6 & (byte)iVar9 | bVar6;
            *(byte *)(iVar5 + 0x62141) = bVar6;
            *(byte *)(iVar5 + 0x62142) = bVar6 + bVar7 + -0x34;
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          }
        }
        pbVar11 = (byte *)(iVar4 + 0x3918);
        cVar2 = PeekPacketChecksumBool();
        if ((cVar2 == '\0') && (iVar5 = FUN_004511b0(local_15ac,(int)(&DAT_006a7f88 + g_clientContext)), iVar5 != 0)) {
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = _rand();
          *pbVar11 = (byte)iVar5;
          iVar5 = _rand();
          *(byte *)(iVar4 + 0x3919) = (byte)iVar5;
          bVar7 = '\x01' << (*pbVar11 & 7);
          bVar7 = ~bVar7 & (byte)iVar5 | bVar7;
          *(byte *)(iVar4 + 0x3919) = bVar7;
          *(byte *)(iVar4 + 0x391a) = bVar7 + *pbVar11 + -0x34;
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        uVar3 = PeekPacketChecksumState((void *)(local_158c));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        FUN_00436dc0(local_15ac,local_15c0,uVar3);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar5 = PeekPacketChecksumState((void *)(local_158c));
        local_159d = iVar5 == 0;
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        if (local_159d != '\0') {
          EncodeChecksumDeltaShr(iVar4 + 0x6ac,local_ce4,8);
          local_4 = 10;
          local_15b8 = EncodeChecksumDeltaShr(iVar4 + 0x488,local_f08,8);
          local_4 = 0xb;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = PeekPacketChecksumState((void *)(local_ce4));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_15b8 = PeekPacketChecksumState((void *)(local_f08));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          FUN_00436bd0(local_15ac - local_15b8,local_15c0 - iVar5,*(undefined4 *)(iVar4 + 0x1e50));
          iVar5 = (*(int *)(local_f08 + 0x14));
          local_4 = 10;
          if ((*(int *)(local_f08 + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_f08 + 0x14),&g_valueGuardKeyTable);
            local_15b8 = iVar5;
            TreeLowerBound(local_1580,&g_valueGuardMap);
            iVar4 = local_159c;
          }
          iVar5 = (*(int *)(local_ce4 + 0x14));
          local_4 = 0xffffffff;
          if ((*(int *)(local_ce4 + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_ce4 + 0x14),&g_valueGuardKeyTable);
            local_15b8 = iVar5;
            TreeLowerBound(local_1580,&g_valueGuardMap);
            iVar4 = local_159c;
          }
        }
        iVar5 = local_15ac;
        cVar2 = PeekPacketChecksumBool((byte *)(iVar4 + 0x3918));
        if (cVar2 != '\0') {
          FUN_00436ec0(iVar5,local_15c0);
        }
        iVar9 = local_15ac;
        iVar10 = local_15c0;
        if ((((-1 < iVar5) && (iVar5 < *(int *)(&g_nCameraBoundX + g_clientContext))) && (-1 < local_15c0)
            ) && ((local_15c0 < *(int *)(&g_nCameraBoundY + g_clientContext) &&
                  (*(char *)(*(int *)(&g_nCameraBoundX + g_clientContext) * local_15c0 +
                             *(int *)(&DAT_006a773c + g_clientContext) + iVar5) != '\0')))) {
          FUN_00458920(&local_15c4,&local_15c0,local_1598,local_1594,local_1590,local_1584,
                       local_1588,1,0,param_2);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x10), local_15c4 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x99), local_15c0 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x3d5), local_15c4);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x45e), local_15c0);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = _rand();
          *(char *)(iVar4 + 0xf45) = (char)iVar5;
          iVar5 = _rand();
          *(byte *)(iVar4 + 0xf46) = (byte)iVar5;
          bVar7 = '\x01' << (*(byte *)(iVar4 + 0xf45) & 7);
          bVar7 = ~bVar7 & (byte)iVar5 | bVar7;
          *(byte *)(iVar4 + 0xf46) = bVar7;
          *(byte *)(iVar4 + 0xf47) = bVar7 + *(char *)(iVar4 + 0xf45) + -0x34;
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (DAT_0079352a != '\0') {
            uVar3 = 0xf002;
            goto LAB_0049fcc1;
          }
          break;
        }
        cVar2 = HitTestMobilesAndJewels(local_15c0,(int)(&DAT_006a7f88 + g_clientContext),local_15ac);
        if (cVar2 != '\0') {
          FUN_00458920(&local_15c4,&local_15c0,local_1598,local_1594,local_1590,local_1584,
                       local_1588,0,1,param_2);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar4 = local_159c;
          EncodeOutgoingPacketField((void *)(param_1 + 0x10), local_15c4 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x99), local_15c0 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x3d5), local_15c4);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x45e), local_15c0);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          bVar12 = DAT_0079352a != '\0';
          *(undefined1 *)(iVar4 + 0x3fa0) = 1;
          if (bVar12) {
            QueueBroadcastEvent(0xf003,(int)&g_replayContext);
            (&g_abBroadcastEventBuffer)[g_dwBroadcastEventCursor] = *(undefined1 *)(iVar4 + 8);
            goto LAB_0049fcda;
          }
          break;
        }
        if (((iVar9 < 0) || (*(int *)(&g_nCameraBoundX + g_clientContext) + -1 < iVar9)) ||
           ((iVar10 < -0x113 || (*(int *)(&g_nCameraBoundY + g_clientContext) + -1 < iVar10))))
        goto LAB_0049fb78;
        local_1594 = local_1594 + param_2;
        local_15ac = iVar9 + local_1578;
      } while (local_1594 < local_15a8[0]);
    }
  }
  else {
    local_1584 = ((local_1598 - local_1590) * 0x10000) / iVar10;
    local_1588 = local_1590 << 0x10;
    if (local_158c < iVar4) {
      local_15ac = iVar4 + -1;
      local_1590 = -1;
    }
    else {
      local_15ac = iVar4 + 1;
      local_1590 = 1;
    }
    local_15b8 = iVar9 + param_2;
    local_1594 = 0;
    if (0 < local_15b8) {
      local_15b4 = local_1584 * param_2;
      local_158c = local_159c + 0x15c0;
      local_1578 = local_1590 * param_2;
      local_1598 = local_15ac;
      do {
        local_1588 = local_1588 + local_15b4;
        local_15c0 = local_15ac;
        local_15c4 = local_1588 >> 0x10;
        if ((local_15ad == '\0') &&
           (iVar5 = FUN_004510f0(local_15c4,(int)(&DAT_006a7f88 + g_clientContext)), iVar4 = local_158c, iVar5 != 0)) {
          local_15a8[0] = EncodeChecksumDeltaAdd(local_158c,local_1350,param_2);
          local_4 = 0xc;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_15a8[0] = PeekPacketChecksumState((void *)(local_1350));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (local_15a8[0] < 0x14) {
LAB_0049f398:
            local_15a8[0] = EncodeChecksumDeltaAdd(iVar4,local_f08,param_2);
            local_4 = 0xe;
            local_15bc = local_15bc | 0x10;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            local_15a8[0] = PeekPacketChecksumState((void *)(local_f08));
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            if (local_15a8[0] < 0x14) {
              uVar3 = 0x14;
            }
            else {
              EncodeChecksumDeltaAdd(iVar4,local_ce4,param_2);
              local_4 = 0xf;
              local_15bc = local_15bc | 0x20;
              EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
              uVar3 = PeekPacketChecksumState((void *)(local_ce4));
              LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            }
          }
          else {
            local_15a8[0] = EncodeChecksumDeltaAdd(iVar4,local_112c,param_2);
            local_4 = 0xd;
            local_15bc = local_15bc | 8;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            local_15a8[0] = PeekPacketChecksumState((void *)(local_112c));
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            if (local_15a8[0] < 0x65) goto LAB_0049f398;
            uVar3 = 100;
          }
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(local_158c), uVar3);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar4 = (*(int *)(local_ce4 + 0x14));
          local_4 = 0xe;
          if (((local_15bc & 0x20) != 0) && (local_15bc = local_15bc & 0xffffffdf, (*(int *)(local_ce4 + 0x14)) != 0))
          {
            ScrambleChecksumGuardBytes();
            local_15a8[0] = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_f08 + 0x14));
          local_4 = 0xd;
          if (((local_15bc & 0x10) != 0) && (local_15bc = local_15bc & 0xffffffef, (*(int *)(local_f08 + 0x14)) != 0))
          {
            ScrambleChecksumGuardBytes();
            local_15a8[0] = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_112c + 0x14));
          local_4 = 0xc;
          if (((local_15bc & 8) != 0) && (local_15bc = local_15bc & 0xfffffff7, (*(int *)(local_112c + 0x14)) != 0)) {
            ScrambleChecksumGuardBytes();
            local_15a8[0] = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          iVar4 = (*(int *)(local_1350 + 0x14));
          local_4 = 0xffffffff;
          if ((*(int *)(local_1350 + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_1350 + 0x14),&g_valueGuardKeyTable);
            local_15a8[0] = iVar4;
            TreeLowerBound(local_1580,&g_valueGuardMap);
          }
          local_15ad = '\x01';
        }
        iVar4 = local_159c;
        pbVar11 = (byte *)(local_159c + 0xf4c);
        cVar2 = DecodeGuardedBool(pbVar11);
        if (cVar2 != '\0') {
          iVar5 = FUN_00451030(local_15c4,(int)(&DAT_006a7f88 + g_clientContext));
          *(int *)(iVar4 + 0xf48) = iVar5;
          if (iVar5 != 0) {
            *(undefined4 *)(iVar4 + 0xf50) = 0;
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar5 = _rand();
            *pbVar11 = (byte)iVar5;
            iVar5 = _rand();
            bVar7 = *pbVar11;
            *(byte *)(iVar4 + 0xf4d) = (byte)iVar5;
            bVar6 = '\x01' << (bVar7 & 7);
            bVar7 = *pbVar11;
            bVar6 = ~bVar6 & (byte)iVar5 | bVar6;
            *(byte *)(iVar4 + 0xf4d) = bVar6;
            *(byte *)(iVar4 + 0xf4e) = bVar6 + bVar7 + -0x34;
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar5 = g_clientContext;
            pbVar11 = (byte *)(g_clientContext + 0x62140);
            EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
            iVar9 = _rand();
            *pbVar11 = (byte)iVar9;
            iVar9 = _rand();
            bVar7 = *pbVar11;
            *(byte *)(iVar5 + 0x62141) = (byte)iVar9;
            bVar6 = '\x01' << (bVar7 & 7);
            bVar7 = *pbVar11;
            bVar6 = ~bVar6 & (byte)iVar9 | bVar6;
            *(byte *)(iVar5 + 0x62141) = bVar6;
            *(byte *)(iVar5 + 0x62142) = bVar6 + bVar7 + -0x34;
            LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          }
        }
        pbVar11 = (byte *)(iVar4 + 0x3918);
        cVar2 = PeekPacketChecksumBool();
        if ((cVar2 == '\0') && (iVar5 = FUN_004511b0(local_15c4,(int)(&DAT_006a7f88 + g_clientContext)), iVar5 != 0)) {
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = _rand();
          *pbVar11 = (byte)iVar5;
          iVar5 = _rand();
          *(byte *)(iVar4 + 0x3919) = (byte)iVar5;
          bVar7 = '\x01' << (*pbVar11 & 7);
          bVar7 = ~bVar7 & (byte)iVar5 | bVar7;
          *(byte *)(iVar4 + 0x3919) = bVar7;
          *(byte *)(iVar4 + 0x391a) = bVar7 + *pbVar11 + -0x34;
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        }
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        uVar3 = PeekPacketChecksumState((void *)(local_158c));
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        FUN_00436dc0(local_15c4,local_15ac,uVar3);
        EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        iVar5 = PeekPacketChecksumState((void *)(local_158c));
        local_159d = iVar5 == 0;
        LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
        if (local_159d != '\0') {
          EncodeChecksumDeltaShr(iVar4 + 0x6ac,local_230,8);
          local_4 = 0x10;
          local_15a8[0] = EncodeChecksumDeltaShr(iVar4 + 0x488,local_678,8);
          local_4 = 0x11;
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = PeekPacketChecksumState((void *)(local_230));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          local_15a8[0] = PeekPacketChecksumState((void *)(local_678));
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          FUN_00436bd0(local_15c4 - local_15a8[0],local_15ac - iVar5,*(undefined4 *)(iVar4 + 0x1e50)
                      );
          iVar5 = (*(int *)(local_678 + 0x14));
          local_4 = 0x10;
          if ((*(int *)(local_678 + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_678 + 0x14),&g_valueGuardKeyTable);
            local_15a8[0] = iVar5;
            TreeLowerBound(local_1580,&g_valueGuardMap);
            iVar4 = local_159c;
          }
          iVar5 = (*(int *)(local_230 + 0x14));
          local_4 = 0xffffffff;
          if ((*(int *)(local_230 + 0x14)) != 0) {
            ScrambleChecksumGuardBytes(*(int *)(local_230 + 0x14),&g_valueGuardKeyTable);
            local_15a8[0] = iVar5;
            TreeLowerBound(local_1580,&g_valueGuardMap);
            iVar4 = local_159c;
          }
        }
        iVar5 = local_15c4;
        cVar2 = PeekPacketChecksumBool((byte *)(iVar4 + 0x3918));
        if (cVar2 != '\0') {
          FUN_00436ec0(iVar5,local_15ac);
        }
        iVar10 = local_15ac;
        iVar9 = local_15c4;
        if ((((-1 < iVar5) && (iVar5 < *(int *)(&g_nCameraBoundX + g_clientContext))) && (-1 < local_15ac)
            ) && ((local_15ac < *(int *)(&g_nCameraBoundY + g_clientContext) &&
                  (*(char *)(local_15ac * *(int *)(&g_nCameraBoundX + g_clientContext) +
                             *(int *)(&DAT_006a773c + g_clientContext) + iVar5) != '\0')))) {
          FUN_00458a00(&local_15c4,&local_15c0,local_1598,local_1594,local_1590,local_1584,
                       local_1588,1,0,param_2);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x10), local_15c4 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x99), local_15c0 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x3d5), local_15c4);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x45e), local_15c0);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar5 = _rand();
          *(char *)(iVar4 + 0xf45) = (char)iVar5;
          iVar5 = _rand();
          *(byte *)(iVar4 + 0xf46) = (byte)iVar5;
          bVar7 = '\x01' << (*(byte *)(iVar4 + 0xf45) & 7);
          bVar7 = ~bVar7 & (byte)iVar5 | bVar7;
          *(byte *)(iVar4 + 0xf46) = bVar7;
          *(byte *)(iVar4 + 0xf47) = bVar7 + *(char *)(iVar4 + 0xf45) + -0x34;
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          if (DAT_0079352a != '\0') {
            QueueBroadcastEvent(0xf002,(int)&g_replayContext);
            (&g_abBroadcastEventBuffer)[g_dwBroadcastEventCursor] = *(undefined1 *)(iVar4 + 8);
            piVar1 = (int *)(&DAT_00e9aacd + g_dwBroadcastEventCursor);
            g_dwBroadcastEventCursor = g_dwBroadcastEventCursor + 1;
            *piVar1 = local_15c4;
            piVar1 = (int *)(&DAT_00e9aad0 + g_dwBroadcastEventCursor);
            g_dwBroadcastEventCursor = g_dwBroadcastEventCursor + 4;
            *piVar1 = local_15c0;
            goto LAB_0049fd06;
          }
          break;
        }
        cVar2 = HitTestMobilesAndJewels(local_15ac,(int)(&DAT_006a7f88 + g_clientContext),local_15c4);
        if (cVar2 != '\0') {
          FUN_00458a00(&local_15c4,&local_15c0,local_1598,local_1594,local_1590,local_1584,
                       local_1588,0,1,param_2);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          iVar4 = local_159c;
          EncodeOutgoingPacketField((void *)(param_1 + 0x10), local_15c4 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x99), local_15c0 << 8);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x3d5), local_15c4);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          EncodeOutgoingPacketField((void *)(param_1 + 0x45e), local_15c0);
          LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
          bVar12 = DAT_0079352a != '\0';
          *(undefined1 *)(iVar4 + 0x3fa0) = 1;
          if (bVar12) {
            uVar3 = 0xf003;
            goto LAB_0049fcc1;
          }
          break;
        }
        if (((iVar9 < 0) || (*(int *)(&g_nCameraBoundX + g_clientContext) + -1 < iVar9)) ||
           ((iVar10 < -0x113 || (*(int *)(&g_nCameraBoundY + g_clientContext) + -1 < iVar10))))
        goto LAB_0049fb78;
        local_1594 = local_1594 + param_2;
        local_15ac = iVar10 + local_1578;
      } while (local_1594 < local_15b8);
    }
  }
LAB_0049fd17:
  *unaff_FS_OFFSET = local_c;
  return;
LAB_0049fb78:
  iVar5 = *(int *)(&g_nCameraBoundX + g_clientContext) + -1;
  iVar4 = iVar5;
  if (iVar9 <= iVar5) {
    iVar4 = iVar9;
  }
  if (iVar4 < 0) {
    local_15c4 = 0;
  }
  else if (iVar5 < iVar9) {
    local_15c4 = iVar5;
  }
  iVar5 = *(int *)(&g_nCameraBoundY + g_clientContext) + -1;
  iVar4 = iVar5;
  if (iVar10 <= iVar5) {
    iVar4 = iVar10;
  }
  if (iVar4 < -0x113) {
    local_15c0 = -0x113;
  }
  else if (iVar5 < iVar10) {
    local_15c0 = iVar5;
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar4 = local_159c;
  EncodeOutgoingPacketField((void *)(param_1 + 0x10), local_15c4 << 8);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EncodeOutgoingPacketField((void *)(param_1 + 0x99), local_15c0 << 8);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EncodeOutgoingPacketField((void *)(param_1 + 0x3d5), local_15c4);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EncodeOutgoingPacketField((void *)(param_1 + 0x45e), local_15c0);
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar5 = _rand();
  *(char *)(iVar4 + 0xf45) = (char)iVar5;
  iVar5 = _rand();
  *(byte *)(iVar4 + 0xf46) = (byte)iVar5;
  bVar7 = '\x01' << (*(byte *)(iVar4 + 0xf45) & 7);
  bVar7 = ~bVar7 & (byte)iVar5 | bVar7;
  *(byte *)(iVar4 + 0xf46) = bVar7;
  *(byte *)(iVar4 + 0xf47) = bVar7 + *(char *)(iVar4 + 0xf45) + -0x34;
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  if (DAT_0079352a != '\0') {
    uVar3 = 0xf004;
LAB_0049fcc1:
    QueueBroadcastEvent(uVar3,(int)&g_replayContext);
    (&g_abBroadcastEventBuffer)[g_dwBroadcastEventCursor] = *(undefined1 *)(iVar4 + 8);
LAB_0049fcda:
    piVar1 = (int *)(&DAT_00e9aacd + g_dwBroadcastEventCursor);
    g_dwBroadcastEventCursor = g_dwBroadcastEventCursor + 1;
    *piVar1 = local_15c4;
    piVar1 = (int *)(&DAT_00e9aad0 + g_dwBroadcastEventCursor);
    g_dwBroadcastEventCursor = g_dwBroadcastEventCursor + 4;
    *piVar1 = local_15c0;
LAB_0049fd06:
    g_dwBroadcastEventCursor = g_dwBroadcastEventCursor + 4;
    BroadcastQueuedEvent((int)&g_replayContext);
  }
  goto LAB_0049fd17;
}

