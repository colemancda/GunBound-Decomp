/* DetonateShot2_Bullet2 - 0x004a4950 in the original binary.
 *
 * RENAMED (2026-08-16, from FUN_004a4950): CProjectile virtual slot 6
 * (DetonateProjectile, base impl 0x4572b0) override for mobile type 1's
 * shot 2 (weapon index 1, texture bullet2s); vtable 0x5564f0, ctor
 * InitShot2_Bullet2 reached only through SpawnPrimaryShot's case 1. Type/role
 ...
 * (0x4a4950-0x4a61f8) - NOT order-zipped: the worklist was flagged
 * MISMATCH because its 78-peek/25-encode orig lists include 14+2 sites
 * of the UNPORTED neighbor function at 0x4a6200/0x4a6840/0x4a6920 (address-containment
 ...
 *     local_cf8 pointer copies (stores at 0x4a498c/0x4a49c5).
 *   - the argless Encodes had also dropped their VALUE pushes: each is
 *     the preceding Peek's return (captured into uVar5/iVar4), the
 *     stanza sites at 0x4a4ad1-0x4a4b6e add the stanza-head peek
 *     (local_d14), and 0x4a4d0f/0x4a4d39 use ((angle-peek)<<8)+peek
 *     where the first term is FloatToInt64's 0x4a4cae return (was a
 *     discarded call at old L182 - now captured in iVar4).
 *   - chained-return cells (helper returns its cell in EAX, decompile
 *     discarded it): EncodeChecksumDeltaMod 0x4a4bc9 -> peek 0x4a4be1,
 *     EncodeChecksumPairDiff 0x4a4e11 -> peek 0x4a4e2c,
 *     EncodeChecksumDeltaShr 0x4a5b95/0x4a5c12/0x4a5feb/0x4a600c ->
 *     peeks 0x4a5bb0/0x4a5c2d/0x4a6024/0x4a603d; captured in uVar5/6.
 ...
 *     cells' earlier peeks: [esp+0x2c]=local_d04 (0x4a4d5e store) and
 *     [esp+0x24]=local_d10[0] (0x4a4d7b store), cells matching their
 *     peek sites 0x4a4d54/0x4a4d71 exactly.
 *   - global cells: &DAT_00e9bed8/&DAT_00e55ab8/&DAT_007949c8
 *     (immediates), g_clientContext+0x5b85c (mov edi,[0x5b3484];
 *     add edi,0x5b85c @0x4a593b) and +0xebcbc (lea @0x4a5169).
 ...
 *   - ESI holds param_1 until 0x4a5b20 (the terminal _rand block), so
 *     all esi-based cells on the edited paths are param_1-relative;
 *     EDI/EBX cells back-tracked per site incl. across callee-saved
 *     call spans (e.g. lea edi,[esi+0xf54] @0x4a4fc1 covering the four
 *     peeks 0x4a4fc9-0x4a5130).
 */
#include "ghidra_types.h"


void __fastcall DetonateShot2_Bullet2(int *param_1)
...
LAB_004a4a60:
    ...
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar5 = PeekPacketChecksumState((void *)local_d00);
    EncodeOutgoingPacketField((void *)(param_1 + 0x113e),uVar5);
    LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  ...
  uVar5 = PeekPacketChecksumState((void *)(param_1 + 0x10b5));
  ...
  (*pcVar18)();
  uVar5 = EncodeChecksumPairDiff(param_1 + 0x10,auStack_cf4,param_1 + 0x113e);
  uStack_14 = 2;
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  ...
  EncodeOutgoingPacketField((void *)(param_1 + 0x113e),uVar6);
  puVar28 = &g_valueGuardLock;
  (*pcVar18)(&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar6 = PeekPacketChecksumState((void *)(param_1 + 0x99));
  EncodeOutgoingPacketField((void *)(param_1 + 0x11c7),uVar6);
