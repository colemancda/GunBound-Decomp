/* DetonateSuperShot_Bullet12 - 0x004ae3a0 in the original binary.
 *
 * RENAMED (2026-08-16, from FUN_004ae3a0): CProjectile virtual slot 6
 * (DetonateProjectile, base impl 0x4572b0) override for mobile type 11's
 * super shot (texture bullet12p; vtable 0x55661c, ctor InitSuperShot_Bullet12
 * reached only through SpawnSuperShot's case 11). Type/role CONFIRMED via
 ...
 * cell at all 11 argless PeekPacketChecksumState() calls.  this=ESI:
 * param_1+0x10/0x99/0x122/0x1ab/0x234/0x3d5/0x45e (byte 0x40/0x264/
 * 0x488/0x6ac/0x8d0/0xf54/0x1178).  The two peeks after the
 * EncodeChecksumDeltaShr calls (0x4ae7fe / 0x4ae85b) read the helper's
 * arg2 scratch auStack_ac0 (the helpers return arg2 - see
 * tools/sweep_guard_instructions.md); the Encodes right after them,
 * already fixed 2026-07-15, write back into param_1+0x3d5 / +0x45e.
 */
#include "ghidra_types.h"


void __fastcall DetonateSuperShot_Bullet12(int *param_1)

{
  ...
  *unaff_FS_OFFSET = &uStack_c;
  ...
  if (cVar6 == '\0') {
     ...
     * tools/encodeoutgoingpacketfield_sites.json. */
    EnterCriticalSection((LPCRITICAL_SECTION)&DAT_005a9068);
    uVar11 = PeekPacketChecksumState((void *)(auStack_ac0));
...
LAB_004ae9a8:
  ...
  *(undefined1 *)(param_1 + 5) = 1;
  ...
  if (param_1[9] == 0) {
     ...
     * arguments (orig 0x4ae642 xor cl,cl / 0x4ae644 mov edx,1).  param_1 is the randomise flag,
    ...
    SpawnSuperFlameEffect(0,1,uVar9,uVar11,uVar10,pcVar19);
  }
