/* DetonateSuperShot_Bullet1 - 0x0044d9b0 in the original binary.
 *
 * RENAMED (2026-08-16, from FUN_0044d9b0): CProjectile virtual slot 6
 * (DetonateProjectile, base impl 0x4572b0) override for mobile type 0's
 * super shot (texture bullet1p; vtable 0x555ac4, ctor InitSuperShot_Bullet1 reached
 * only through SpawnSuperShot's case 0). Type/role CONFIRMED via vtable
 * geometry (tools/projectile_class_map.py); the mobile NAME is not
 * confirmable (no mobile-type->name map in the repo).
 *
 * Body is a raw/near-verbatim Ghidra port, not hand-verified. See
 * src/README.md's "Raw/verbatim ports" section for status.
 *
 * DROPPED-CELL FIX (2026-08-16, CValueGuard sweep): recovered the guard
 * cell at all 20 argless PeekPacketChecksumState() calls (worklist 20:20).
 * Head-twin of FUN_00452cc0 / tail-twin of DetonateShot1_Bullet7: this=ESI, cells
 * param_1+0x10/0x99/0x122/0x1ab/0x234/0x3d5/0x45e, globals 0xe55ab8 /
 * 0x7949c8.  Chained returns: the six EncodeChecksumDeltaDiv results
 * (each -> frame[0x10] -> `mov eax,[esp+0x10]; call Peek`) - one was
 * already kept in unaff_EBP by the decompile, the other five are captured
 * in iDelta; the two EncodeChecksumDeltaShr results (0x44e365/0x44e3e2,
 * `mov edi,eax` -> `mov eax,edi; call Peek`) in pvDelta.
 */
#include "ghidra_types.h"


void __fastcall DetonateSuperShot_Bullet1(int *param_1)

{
  /* Ghidra artifact: raw stack reference the decompiler could not
   * map to a named local; declared so the raw port parses. */
  undefined stack0xfffff518;
  undefined stack0xfffff51c;
  int *piVar1;
  bool bVar2;
  bool bVar3;
  bool bVar4;
  char cVar5;
  undefined3 extraout_EAX;
  void *pvDelta;
  int iDelta;
  int iVar6;
  undefined4 uVar7;
  int iVar8;
  undefined4 uVar9;
  char extraout_var;
  uint uVar10;
  int iVar11;
  int iVar12;
  int iVar13;
  int iVar14;
  char *pcVar15;
  code *pcVar16;
  undefined4 unaff_EBP;
  undefined4 *unaff_FS_OFFSET;
  undefined4 uVar17;
  undefined4 uVar18;
  undefined4 uVar19;
  int iStack_ae0;
  undefined4 uStack_adc;
  int local_ad8;
  int iImpactX;
  undefined1 auStack_ad4 [4];
  undefined4 uStack_ad0;
  undefined1 auStack_acc [4];
  undefined1 auStack_ac8 [4];
  undefined1 auStack_ac4 [4];
  int iStack_ac0;
  int iStack_abc;
  int iStack_ab8;
  int iStack_ab4;
  int iStack_ab0;
  int iStack_aac;
  undefined1 auStack_8b0 [4];
  undefined1 auStack_8ac [4];
  undefined1 auStack_8a8 [4];
  undefined1 auStack_8a4 [4];
  undefined1 auStack_8a0 [4];
  int iStack_89c;
  int iStack_898;
  int iStack_894;
  int iStack_890;
  int iStack_88c;
  int iStack_888;
  undefined1 auStack_68c [548];
  undefined1 auStack_468 [548];
  undefined1 auStack_244 [556];
  undefined4 uStack_18;
  undefined4 uStack_14;
  undefined4 uStack_10;
  undefined4 uStack_c;
  undefined1 *puStack_8;
  undefined4 uStack_4;
  
  uStack_4 = 0xffffffff;
  puStack_8 = &LAB_0054061d;
  uStack_c = *unaff_FS_OFFSET;
  *unaff_FS_OFFSET = &uStack_c;
  local_ad8 = 0;
  if ((param_1[0xfe7] != -1) && (iVar6 = param_1[0xfe7] + 1, param_1[0xfe7] = iVar6, iVar6 == 0x24))
  {
    param_1[0xfe7] = -1;
    (**(code **)(*param_1 + 4))(s_change_00555af0);
  }
  pcVar16 = (code *)LeaveCriticalSection;
  if ((char)param_1[0xd] == '\x01') {
    if (param_1[9] == 1) {
      (**(code **)(*param_1 + 4))(s_cnormal_00555ae8);
      uVar7 = InitGuardedChecksumSlot(param_1 + 0x795,auStack_ac4,param_1 + 0xcf2);
      puStack_8 = (undefined1 *)0x0;
      iDelta = EncodeChecksumDeltaDiv(uVar7,auStack_8a0,100);
      puStack_8 = 1;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(iDelta));
      /* FIXED (2026-07-15): dropped `self` arg - angr-confirmed at
       * 0x44da80 (edi set by `lea edi,[esi+0x1e54]` at the
       * InitGuardedChecksumSlot call above, unchanged since) the cell is
       * param_1+0x795 (== esi+0x1e54), the same cell just passed to
       * InitGuardedChecksumSlot/EncodeChecksumDeltaDiv above. Same
       * evidence pattern for all 6 InitGuardedChecksumSlot-guarded cells
       * below. See tools/encodeoutgoingpacketfield_sites.json. */
      EncodeOutgoingPacketField(param_1 + 0x795, uVar7);
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      puStack_8 = (undefined1 *)0;
      if (iStack_88c != 0) {
        ScrambleChecksumGuardBytes(iStack_88c,&g_valueGuardKeyTable);
        TreeLowerBound(&local_ad8,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      puStack_8 = (undefined1 *)0xffffffff;
      if (iStack_ab0 != 0) {
        ScrambleChecksumGuardBytes(iStack_ab0,&g_valueGuardKeyTable);
        TreeLowerBound(&local_ad8,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uVar7 = InitGuardedChecksumSlot(param_1 + 0x81e,auStack_8a0,param_1 + 0xcf2);
      puStack_8 = (undefined1 *)0x2;
      iDelta = EncodeChecksumDeltaDiv(uVar7,auStack_ac4,100);
      puStack_8 = (undefined1 *)3;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(iDelta));
      EncodeOutgoingPacketField(param_1 + 0x81e, uVar7);
      (*pcVar16)(&g_valueGuardLock);
      uStack_c = 2;
      if (iStack_ab4 != 0) {
        ScrambleChecksumGuardBytes(iStack_ab4,&g_valueGuardKeyTable);
        TreeLowerBound(&uStack_adc,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uStack_c = 0xffffffff;
      if (iStack_890 != 0) {
        ScrambleChecksumGuardBytes(iStack_890,&g_valueGuardKeyTable);
        TreeLowerBound(&uStack_adc,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uVar7 = InitGuardedChecksumSlot(param_1 + 0x8a7,auStack_8a4,param_1 + 0xcf2);
      uStack_c = 4;
      unaff_EBP = EncodeChecksumDeltaDiv(uVar7,auStack_ac8,100);
      uStack_c = 5;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(unaff_EBP));
      EncodeOutgoingPacketField(param_1 + 0x8a7, uVar7);
      (*pcVar16)(&g_valueGuardLock);
      uStack_10 = 4;
      if (iStack_ab8 != 0) {
        ScrambleChecksumGuardBytes(iStack_ab8,&g_valueGuardKeyTable);
        TreeLowerBound(&iStack_ae0,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uStack_10 = 0xffffffff;
      if (iStack_894 != 0) {
        ScrambleChecksumGuardBytes(iStack_894,&g_valueGuardKeyTable);
        TreeLowerBound(&iStack_ae0,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uVar7 = InitGuardedChecksumSlot(param_1 + 0x930,auStack_8a8,param_1 + 0xd7b);
      uStack_10 = 6;
      iDelta = EncodeChecksumDeltaDiv(uVar7,auStack_acc,100);
      uStack_10 = 7;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(iDelta));
      EncodeOutgoingPacketField(param_1 + 0x930, uVar7);
      (*pcVar16)(&g_valueGuardLock);
      uStack_14 = 6;
      if (iStack_abc != 0) {
        ScrambleChecksumGuardBytes(iStack_abc,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff51c,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uStack_14 = 0xffffffff;
      if (iStack_898 != 0) {
        ScrambleChecksumGuardBytes(iStack_898,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff51c,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uVar7 = InitGuardedChecksumSlot(param_1 + 0x9b9,auStack_8ac,param_1 + 0xd7b);
      uStack_14 = 8;
      iDelta = EncodeChecksumDeltaDiv(uVar7,&uStack_ad0,100);
      uStack_14 = 9;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(iDelta));
      EncodeOutgoingPacketField(param_1 + 0x9b9, uVar7);
      (*pcVar16)(&g_valueGuardLock);
      uStack_18 = 8;
      if (iStack_ac0 != 0) {
        ScrambleChecksumGuardBytes(iStack_ac0,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff518,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uStack_18 = 0xffffffff;
      if (iStack_89c != 0) {
        ScrambleChecksumGuardBytes(iStack_89c,&g_valueGuardKeyTable);
        TreeLowerBound(&stack0xfffff518,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uVar7 = InitGuardedChecksumSlot(param_1 + 0xa42,auStack_8b0,param_1 + 0xd7b);
      uStack_18 = 10;
      iDelta = EncodeChecksumDeltaDiv(uVar7,auStack_ad4,100);
      uStack_18 = 0xb;
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      uVar7 = PeekPacketChecksumState((void *)(iDelta));
      EncodeOutgoingPacketField(param_1 + 0xa42, uVar7);
      (*pcVar16)(&g_valueGuardLock);
      uStack_4 = 10;
      if (iStack_aac != 0) {
        ScrambleChecksumGuardBytes(iStack_aac,&g_valueGuardKeyTable);
        iStack_ae0 = iStack_aac;
        TreeLowerBound(auStack_ad4,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      uStack_4 = 0xffffffff;
      if (iStack_888 != 0) {
        ScrambleChecksumGuardBytes(iStack_888,&g_valueGuardKeyTable);
        iStack_ae0 = iStack_888;
        TreeLowerBound(auStack_ad4,&g_valueGuardMap);
        pcVar16 = (code *)LeaveCriticalSection;
      }
      goto LAB_0044df28;
    }
  }
  else {
LAB_0044df28:
    if (param_1[9] == 1) goto LAB_0044e8d1;
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iStack_ae0 = PeekPacketChecksumState((void *)(param_1 + 0x10));
  iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x122));
  /* FIXED (2026-07-15): dropped `self` args - angr-confirmed at
   * 0x44df59/0x44df8f (`lea edi,[esi+0x40]` / `lea edi,[esi+0x264]`,
   * esi = this file's own param_1) two distinct CValueGuard cells at
   * (int)param_1+0x40 and (int)param_1+0x264. See
   * tools/encodeoutgoingpacketfield_sites.json. */
  EncodeOutgoingPacketField((int)param_1 + 0x40, iVar6 + iStack_ae0);
  (*pcVar16)(&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x99));
  iVar8 = PeekPacketChecksumState((void *)(param_1 + 0x1ab));
  EncodeOutgoingPacketField((int)param_1 + 0x264, iVar8 + iVar6);
  (*pcVar16)(&g_valueGuardLock);
  (**(code **)(*param_1 + 0x14))(8);
  cVar5 = PeekPacketChecksumBool((byte *)param_1 + 0xf45);
  if (cVar5 == '\0') {
    pvDelta = EncodeChecksumDeltaShr(param_1 + 0x10,auStack_acc,8);
    uStack_10 = 0xc;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar7 = PeekPacketChecksumState((void *)(pvDelta));
    /* FIXED (2026-07-15): dropped `self` arg - angr-confirmed at
     * 0x44e38c (`lea edi,[esi+0xf54]`) the cell is (int)param_1+0xf54.
     * See tools/encodeoutgoingpacketfield_sites.json. */
    EncodeOutgoingPacketField((int)param_1 + 0xf54, uVar7);
    (*pcVar16)(&g_valueGuardLock);
    uStack_14 = 0xffffffff;
    if (iStack_abc != 0) {
      ScrambleChecksumGuardBytes(iStack_abc,&g_valueGuardKeyTable);
      TreeLowerBound(&uStack_adc,&g_valueGuardMap);
      pcVar16 = (code *)LeaveCriticalSection;
    }
    pvDelta = EncodeChecksumDeltaShr(param_1 + 0x99,&uStack_ad0,8);
    uStack_14 = 0xd;
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uVar7 = PeekPacketChecksumState((void *)(pvDelta));
    /* FIXED (2026-07-15): dropped `self` arg - angr-confirmed at
     * 0x44e411 (`mov edi,dword ptr [esp+0x10]`) - traced back to
     * `lea edx,[esi+0x1178]; mov [esp+0x14],edx` a few instructions
     * earlier (same physical stack slot once the pending-push offset is
     * accounted for): the cell is (int)param_1+0x1178. See
     * tools/encodeoutgoingpacketfield_sites.json. */
    EncodeOutgoingPacketField((int)param_1 + 0x1178, uVar7);
    (*pcVar16)(&g_valueGuardLock);
    uStack_18 = 0xffffffff;
    if (iStack_ac0 != 0) {
      ScrambleChecksumGuardBytes(iStack_ac0,&g_valueGuardKeyTable);
      TreeLowerBound(&iStack_ae0,&g_valueGuardMap);
      pcVar16 = (code *)LeaveCriticalSection;
    }
    cVar5 = CheckGuardedBoolAnd(param_1[0x3d2] != 0,(byte *)(param_1 + 0x3d3));
    if (cVar5 == '\0') {
      EmitChecksumSum(param_1 + 0x122, param_1 + 0x2bd);
      EmitChecksumSum(param_1 + 0x1ab, param_1 + 0x346);
    }
    else {
      uVar10 = PeekChecksumStateUnderLock(param_1 + 0x122);
      if ((int)((uVar10 ^ (int)uVar10 >> 0x1f) - ((int)uVar10 >> 0x1f)) < 200) {
        cVar5 = PacketChecksumLessThan(param_1 + 0x122,0);
        if (cVar5 == '\0') {
          uVar7 = 200;
        }
        else {
          uVar7 = 0xffffff38;
        }
        QueueOutgoingPacketField(uVar7);
      }
      uVar7 = EncodeChecksumDeltaDiv(param_1[0x3d2] + 0x25c,auStack_468,4);
      uStack_18 = 0xe;
      uVar7 = EncodeChecksumPairDiff(param_1[0x3d2] + 0x38,auStack_68c,uVar7);
      uStack_18 = 0xf;
      bVar3 = false;
      bVar2 = false;
      cVar5 = CompareChecksumPair(param_1 + 0x3d5,uVar7);
      if (cVar5 == '\0') {
        uVar7 = EncodeChecksumDeltaDiv(param_1[0x3d2] + 0x25c,auStack_8b0,4);
        uStack_18 = 0x10;
        uVar7 = EncodeChecksumPairSum(param_1[0x3d2] + 0x38,auStack_ad4,uVar7);
        uStack_18 = 0x11;
        bVar3 = true;
        bVar2 = true;
        cVar5 = CompareChecksumPair(uVar7,param_1 + 0x3d5);
        bVar4 = false;
        if (cVar5 != '\0') goto LAB_0044e580;
      }
      else {
LAB_0044e580:
        bVar4 = true;
      }
      uStack_18 = 0x10;
      if (bVar2) {
        ScrubChecksumGuard();
      }
      uStack_18 = 0xf;
      if (bVar3) {
        ScrubChecksumGuard();
      }
      uStack_18 = 0xe;
      ScrubChecksumGuard();
      uStack_18 = 0xffffffff;
      ScrubChecksumGuard();
      if (bVar4) {
        iVar6 = param_1[0x3d4];
        param_1[0x3d4] = iVar6 + 1;
        if (iVar6 + 1 < 3) {
          uVar7 = EncodeChecksumNegate(param_1 + 0x122,auStack_244);
          uStack_18 = 0x12;
          EncodeChecksumState(uVar7);
          uStack_18 = 0xffffffff;
          ScrubChecksumGuard();
        }
        else {
          SetGuardedBool(0,GB_GUARD_UNRECOVERED);
        }
      }
    }
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    PeekPacketChecksumState((void *)(param_1 + 0x3d5));
    (*pcVar16)(&g_valueGuardLock);
    if ((((SUBFIELD(uStack_adc,3,undefined1) != '\0') ||
         (cVar5 = PacketChecksumGreaterEqual(param_1 + 0x3d5,*(undefined4 *)(&g_nCameraBoundX + g_clientContext)),
         iVar6 = iStack_ae0, cVar5 != '\0')) ||
        (cVar5 = PacketChecksumGreaterEqual(iStack_ae0,*(undefined4 *)(&g_nCameraBoundY + g_clientContext)),
        cVar5 != '\0')) || (cVar5 = PacketChecksumLessThan(iVar6,0xfffffc18), cVar5 != '\0')) {
      *(undefined1 *)(param_1 + 5) = 1;
      cVar5 = PeekPacketChecksumBool();
      if ((cVar5 != '\0') && (cVar5 = InitChecksumSeed(), cVar5 == '\0')) {
        iVar6 = *(int *)(*(int *)(g_clientContext + 0x621e4) + 0x24);
        if ((iVar6 == 5) || (uVar7 = 2, iVar6 == 6)) {
          uVar7 = 0x19;
        }
        QueueOutgoingPacketField(uVar7);
        iVar6 = g_clientContext;
        *(undefined4 *)(&DAT_005f3768 + g_clientContext) = 5;
        *(undefined4 *)(&DAT_005f376c + iVar6) = 0;
      }
    }
    cVar5 = CheckBothGuardedBools((byte *)param_1 + 0xf3f,(byte *)param_1 + 0xf42);
    if (cVar5 != '\0') {
      iVar8 = PeekChecksumStateUnderLock(iStack_ae0);
      iVar11 = PeekChecksumStateUnderLock(param_1 + 0x3d5);
      iVar6 = g_clientContext;
      piVar1 = (int *)(&DAT_006a7708 + g_clientContext);
      cVar5 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
      if ((cVar5 == '\0') && ((&DAT_006a7758)[iVar6] != '\0')) {
        if (((&DAT_006a7736)[iVar6] == '\x01') &&
           ((uVar10 = iVar8 - *(int *)(&g_nCameraY + iVar6) >> 0x1f,
            200 < (int)((iVar8 - *(int *)(&g_nCameraY + iVar6) ^ uVar10) - uVar10) ||
            (uVar10 = iVar11 - *(int *)(&g_nCameraX + iVar6) >> 0x1f,
            300 < (int)((iVar11 - *(int *)(&g_nCameraX + iVar6) ^ uVar10) - uVar10))))) {
          (&DAT_006a7736)[iVar6] = 0;
        }
        iVar13 = 400;
        if (399 < iVar11) {
          iVar13 = iVar11;
        }
        iVar12 = *(int *)(&g_nCameraBoundX + iVar6) + -400;
        if ((iVar13 <= iVar12) && (iVar12 = iVar11, iVar11 < 400)) {
          iVar12 = 400;
        }
        iVar11 = -0x14;
        if (-0x15 < iVar8) {
          iVar11 = iVar8;
        }
        iVar13 = *(int *)(&g_nCameraBoundY + iVar6) + -0x104;
        if ((iVar11 <= iVar13) && (iVar13 = -0x14, -0x15 < iVar8)) {
          iVar13 = iVar8;
        }
        iVar8 = *piVar1 - iVar12;
        *(int *)(&g_nCameraScrollY + iVar6) = iVar13;
        if (((*(int *)(&DAT_006a770c + iVar6) - iVar13) * (*(int *)(&DAT_006a770c + iVar6) - iVar13)
             + iVar8 * iVar8 < 40000) &&
           (((&DAT_006a7734)[iVar6] == '\x01' || ((&DAT_006a7f6c)[iVar6] == '\0')))) {
          *(int *)(&g_nCameraX + iVar6) = iVar12;
          *piVar1 = iVar12;
          *(int *)(&g_nCameraY + iVar6) = iVar13;
          *(int *)(&DAT_006a770c + iVar6) = iVar13;
        }
        if ((((&DAT_006a7734)[iVar6] == '\0') && ((&DAT_006a7f6c)[iVar6] == '\x01')) &&
           ((&DAT_006a7736)[iVar6] == '\0')) {
          iVar8 = *(int *)(&DAT_006a7730 + iVar6);
          iVar11 = iVar8;
          if (iVar12 <= iVar8) {
            iVar11 = iVar12;
          }
          iVar14 = *(int *)(&DAT_006a772c + iVar6);
          if ((iVar14 <= iVar11) && (iVar14 = iVar8, iVar12 <= iVar8)) {
            iVar14 = iVar12;
          }
          *(int *)(&g_nCameraScrollX + iVar6) = iVar14;
        }
        if ((&DAT_006a7734)[iVar6] == '\x01') {
          *(int *)(&g_nCameraScrollX + iVar6) = iVar12;
          *(int *)(&g_nCameraScrollY + iVar6) = iVar13;
        }
      }
    }
    goto LAB_0044e8d1;
  }
  SyncOutgoingChecksumField(param_1 + 0x3d5, param_1[2],param_1 + 0x45e);
  cVar5 = PeekPacketChecksumBool((byte *)param_1 + 0xf3f);
  if (cVar5 != '\0') {
    iVar6 = PeekChecksumStateUnderLock(param_1 + 0x3d5);
    if ((-1 < iVar6) && (iVar6 < *(int *)(&g_nCameraBoundX + g_clientContext))) {
      pcVar15 = (char *)(*(int *)(&DAT_006a773c + g_clientContext) + iVar6);
      iVar6 = 0;
      if (0 < *(int *)(&g_nCameraBoundY + g_clientContext)) {
        do {
          if (*pcVar15 != '\0') goto LAB_0044e02e;
          pcVar15 = pcVar15 + *(int *)(&g_nCameraBoundX + g_clientContext);
          iVar6 = iVar6 + 1;
        } while (iVar6 < *(int *)(&g_nCameraBoundY + g_clientContext));
      }
    }
    iVar6 = 10000;
LAB_0044e02e:
    piVar1 = param_1 + 0x3d5;
    iVar8 = iVar6;
    uVar7 = PeekChecksumStateUnderLock(piVar1);
    FUN_00436070(&DAT_006a7f70 + g_clientContext,uVar7,iVar8);
    iVar8 = 1;
    if (0 < param_1[0xfe5]) {
      iVar8 = param_1[0xfe5];
    }
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uStack_ad0 = PeekPacketChecksumState((void *)(&DAT_00e55ab8));
    (*pcVar16)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    uStack_adc = PeekPacketChecksumState((void *)(&DAT_00e55ab8));
    (*pcVar16)(&g_valueGuardLock);
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    unaff_EBP = PeekPacketChecksumState((void *)(&DAT_007949c8));
    (*pcVar16)(&g_valueGuardLock);
    uVar10 = (uint)*(byte *)(param_1 + 0xf);
    iVar8 = local_ad8 / iVar8;
    uVar17 = 0;
    iVar11 = iStack_ae0;
    uVar7 = uStack_ad0;
    uVar9 = PeekChecksumStateUnderLock(piVar1);
    ApplyBlastDamage(uVar9,iVar6,uVar17,iVar11,iVar8,uVar7,uVar10);
    uVar19 = 0;
    uVar18 = 2;
    uVar17 = 0xff;
    uVar7 = PeekChecksumStateUnderLock(param_1 + 0x5f9);
    uVar9 = 0;
    iImpactX = PeekChecksumStateUnderLock(piVar1);
    PeekPacketChecksumBool((byte *)param_1 + 0xf3c);
    SpawnBlastEffect(iVar6,iImpactX,(char)param_1[0xf],7,0,uVar9,uVar7,uVar17,uVar18,uVar19);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iStack_ae0 = PeekPacketChecksumState((void *)(param_1 + 0x45e));
  (*pcVar16)(&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(param_1 + 0x3d5));
  SUBFIELD(uStack_adc,0,undefined3) = extraout_EAX;
  SUBFIELD(uStack_adc,3,undefined1) = extraout_var;
  (*pcVar16)(&g_valueGuardLock);
  FUN_004398e0(&DAT_006a7f70 + g_clientContext,param_1[2],iStack_ae0,unaff_EBP,param_1,0);
  *(undefined1 *)(param_1 + 5) = 1;
  (**(code **)(*param_1 + 0x20))();
  if (1 < param_1[9]) {
    pcVar15 = s_ssflame1_00555410;
    uVar17 = 0x206c;
    uVar7 = PeekChecksumStateUnderLock(param_1 + 0x45e);
    uVar9 = PeekChecksumStateUnderLock(param_1 + 0x3d5);
    /* RECOVERED (2026-08-19): the two dropped __fastcall register
     * arguments (orig 0x44e1c0 mov cl,1 / 0x44e1c2 mov edx,1).  param_1 is the randomise flag,
     * param_2 the sub-effect index (the callee caps it at < 4).  The third
     * register argument, EAX, needs nothing here: the 2026-08-17 audit proved
     * every site passes ctx+0x6a7f70 and the callee already inlines the
     * resulting +0x6a7f74 gate peek. */
    SpawnSuperFlameEffect(1,1,uVar9,uVar7,uVar17,pcVar15);
  }
  cVar5 = PeekPacketChecksumBool((byte *)param_1 + 0xf3f);
  iVar6 = g_clientContext;
  if ((cVar5 != '\0') && (cVar5 = InitChecksumSeed(), iVar6 = g_clientContext, cVar5 == '\0')) {
    uVar7 = PeekChecksumStateUnderLock(&DAT_007949c8);
    QueueOutgoingPacketField(uVar7);
    iVar6 = g_clientContext;
    *(undefined4 *)(&DAT_005f3768 + g_clientContext) = 5;
    *(undefined4 *)(&DAT_005f376c + iVar6) = 0;
  }
  uVar7 = CompareChecksumPair(iVar6 + 0x5b85c,param_1 + 0xe48);
  cVar5 = CheckGuardedBoolAnd(uVar7);
  if (cVar5 != '\0') {
    EncodeChecksumState(param_1 + 0xe48);
  }
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  PeekPacketChecksumState((void *)(param_1 + 0x234));
  (*pcVar16)(&g_valueGuardLock);
  if ((SUBFIELD(uStack_adc,3,undefined1) == '\0') || (cVar5 = PacketChecksumGreaterThan(param_1 + 0x122,0), cVar5 == '\0')) {
    EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
    iVar6 = PeekPacketChecksumState((void *)(param_1 + 0x234));
    SUBFIELD(uStack_adc,3,undefined1) = 0 < iVar6;
    (*pcVar16)(&g_valueGuardLock);
    if ((SUBFIELD(uStack_adc,3,undefined1) != '\0') && (cVar5 = PacketChecksumLessThan(param_1 + 0x122,0), cVar5 != '\0'))
    goto LAB_0044e2cb;
  }
  else {
LAB_0044e2cb:
    cVar5 = PeekPacketChecksumBool((byte *)param_1 + 0xf4c);
    if (cVar5 == '\0') {
      SetGuardedBool(1,GB_GUARD_UNRECOVERED);
    }
  }
  piVar1 = param_1 + 0xed2;
  cVar5 = CompareChecksumPair(param_1 + 0x3d5,piVar1);
  if (((cVar5 != '\0') && (cVar5 = CompareChecksumPair(piVar1,param_1 + 0xf5b), cVar5 != '\0')) ||
     ((cVar5 = CompareChecksumExceeds(param_1 + 0x3d5,piVar1), cVar5 != '\0' &&
      ((cVar5 = CompareChecksumExceeds(piVar1,param_1 + 0xf5b), cVar5 != '\0' && (5 < param_1[0xed1])))))) {
    SetGuardedBool(1,GB_GUARD_UNRECOVERED);
  }
LAB_0044e8d1:
  *unaff_FS_OFFSET = uStack_c;
  return;
}

