/* DetonateProjectile - 0x004572b0 in the original binary.
 *
 * No confirmed real name/purpose. Raw/near-verbatim port of Ghidra's
 * decompiler output, not hand-verified. See src/README.md's "Raw/
 * verbatim ports" section for status.
 *
 * DROPPED-CELL FIX (2026-08-11, guard sweep; COMPLETED 2026-08-12):
 * recovered the dropped guard-cell pointer at all 55 argless
 * PeekPacketChecksumState() sites and all 12 1-arg
 * EncodeOutgoingPacketField() sites (tools/
 * guard_worklist.json, both lists `clean`; the decompile is
 * goto-shuffled, so every site was landmark-matched - NOT order-zipped -
 * against a full capstone disasm of 0x4572b0-0x4585d0; Peek's cell is
 * EAX, Encode's is EDI at the call). Notably the naive line-order zip
 * would have SWAPPED the four EncodeOutgoingPacketField(0) sites: the C
 * else-branch at lines ~404-410 (markers 1/2, orig 0x45777d/0x4577a9)
 * precedes the if-branch at ~373-379 (markers 3/4, orig
 * 0x45786b/0x457891) in address order. Cell classes: (1) object cells
 * `lea [ebx + 0xNNN]` with ebx = this/[esp+0x30] - expressed as
 * param_1/piVar9/piStack_ad8 + NNN/4 to match each region's existing
 * naming (all alias the same object); (2) the two scratch guard
 * buffers auStack_ac4 ([esp base+0x44]) and auStack_8a0 ([esp
 * base+0x268]), identified from their paired uStack_8a4/iStack_ab0 and
 * uStack_680/uStack_88c zero-stores (frame base = entry-0xb08, Ghidra
 * name = 0xb08 - base offset); (3) globals &DAT_00e55ab8/&DAT_007949c8
 * and g_clientContext+0xebcbc/+0x5b85c; (4) chained-return cells - the
 * two EncodeChecksumDeltaShr (0x40b180) calls at 0x457ff0/0x45806b
 * RETURN their dest cell in EAX (`mov edi,eax`) and the decompile
 * discarded them; those returns are now captured into the existing
 * uVar8 and passed to the paired Peek at lines ~77/~89.
 *
 * The 2026-08-12 resume covered the tail block, which maps 1:1 (31 C
 * sites : 31 orig sites) onto the contiguous address run
 * 0x4579f3-0x457f38 - the only part of the function whose C order does
 * match address order. `ebx` there is the object, reloaded from
 * [esp+0x30] at 0x4579de/0x457b56(edi)/0x457d1a; the identification is
 * pinned by three independent landmarks that the decompile already
 * spells out in object-relative form: `*(char *)(piStack_ad8 + 5) = 1`
 * = 0x4579e7 `mov byte [ebx+0x14],1`, CompareChecksumPair(piStack_ad8 +
 * 0xed2, + 0xf5b) = 0x457f17 push [ebx+0x3d6c]/[ebx+0x3b48], and
 * piStack_ad8[0xed1] < 6 = 0x457f6e `cmp [ebx+0x3b44],5`. Watch out:
 * 0x457af0 does `add ebx,0xf54` and 0x457b02/0x457bcc/0x457c99 do `mov
 * ebx,eax` - after each, a bare `ebx` is NOT the object, so the four
 * later `mov eax,ebx` cells resolve through the [esp+0x28] spill of
 * `lea ebx,[edi+0x1178]` (0x457b5a), not through the object base.
 * Also note (out of scope, flagged): the Peek at 0x457af8 returns into
 * ebx and is pushed as an argument to ApplyCraterExcavation at
 * 0x457b38, but the decompile discarded that return and gives the call
 * 8 args - a dropped-arg fix for a different sweep.
 */
...

{
  ...
  *unaff_FS_OFFSET = &uStack_c;
  ...
  if (cVar4 == '\0') {
    uVar8 = EncodeChecksumDeltaShr(param_1 + 0x10,auStack_ac4,8);
    ...
    uVar8 = EncodeChecksumDeltaShr(param_1 + 0x99,auStack_ac4,8);
...
LAB_004583d9:
    ...
    if (cVar4 != '\0') {
      ...
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar10 = PeekPacketChecksumState((void *)(param_1 + 0x3d5));
      LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
      iVar5 = g_clientContext;
  ...
  if (iVar6 == 4) {
    ...
    while (uVar13 < 0x186ab) {
      if (uVar13 == 0x186aa) {
        ...
        bVar17 = true;
        if (piVar9[2] == 0) goto LAB_00457675;
        break;
      }
...
LAB_0045793d:
        ...
        cVar4 = FUN_004e4fe0(iVar6 - iVar11,&DAT_006a7708 + g_clientContext,&uStack_adc,&local_ad4,2,1,1,iVar12);
        if (cVar4 != '\0') {
          uStack_8a4 = 0;
          iStack_ab0 = 0;
          EncodeOutgoingPacketField((void *)auStack_ac4, 0);
       ...
       * 0x457989/0x45799a. Disasm traces EDI to a `mov 0x28(%esp),%edi`
       * reload whose only prior writer is the list-node found by the
       * traversal loop above (LAB_00457675), i.e. the same value already
       * held in `piVar9` here (piVar9 is not reassigned between that loop
       * and this point). */
      uVar8 = FUN_004ac4d0(piVar9);
      puStack_af4[0xfe9] = uVar8;
      iVar5 = FUN_004ac400(piVar9);
      puStack_af4[0xfe8] = iVar5 * param_1[0xfe5];
      /* guard-cell: proven.  The 0x4579b4 call reads [esp+0x38]; the last
       * write to that slot on EVERY path here is 0x457726 (`mov [esp+0x38],
       * eax`, eax = ctx+0x6a7f74 from the [esp+0x18] spill of `lea ecx,
       * [eax+0x6a7f70]` at 0x457592+4) - no jump enters (0x457726, 0x4579b4]
       * from outside, checked over the whole function.  The 4th read of the
       * same ctx flag in this function. */
      cVar4 = PeekPacketChecksumBool((byte *)(g_clientContext + 0x6a7f74));
      if (cVar4 == '\0') {
...
LAB_004579de:
  *(undefined1 *)(piStack_ad8 + 5) = 1;
  ...
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  iCraterX = PeekPacketChecksumState((void *)(piStack_ad8 + 0x3d5));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  ...
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  SpawnCrashEffect(uVar18, uVar8, piVar9[0xfe4]);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  uVar8 = PeekPacketChecksumState((void *)(piVar9 + 0x45e));
  LeaveCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  EnterCriticalSection((LPCRITICAL_SECTION)&g_valueGuardLock);
  ...
  if ((iVar6 <= iVar5) ||
    ...
    if ((iVar5 <= iVar6) ||
       ((cVar4 = CompareChecksumExceeds(piStack_ad8 + 0xed2,piStack_ad8 + 0xf5b), cVar4 == '\0' ||
        (piStack_ad8[0xed1] < 6)))) goto LAB_004585ba;
  }
  iVar5 = g_clientContext;
