...
| 11 | `0x4c1c90` | `RenderWindGaugeTick` — draws three things each tick, gated on a per-difficulty/stage flag: `DrawStageDecorationParallax`/`DrawStageDecorationBase` (the two-layer parallax stage-decoration object, see `LoadStageDecorationSet`), then the wind gauge (rotating 8-frame direction vane + digit-sprite strength number) via `DrawWindGauge` (`0x406990`). All three composite through the span-based HUD compositor (see ARCHITECTURE.md "The span-based HUD compositor"). |
